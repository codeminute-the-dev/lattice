#!/usr/bin/env bash
#
# End-to-end smoke tests for the Lattice RPC proxy sidecar.
#
...
#
# Usage:
#   ./proxy/proxy_test.sh --node-rpc=node1.testnet.lattice.codeminute.dev:44111 \
...
CONTAINER_NAME="latticed-proxy-test-$$"
CA_CERT=""
PASS_COUNT=0
FAIL_COUNT=0
...
echo "=== Lattice RPC Proxy Smoke Tests ==="
echo "  Node:  $NODE_RPC"
...
CADDYFILE=$(mktemp /tmp/lattice-caddyfile-XXXXXX)
cat > "$CADDYFILE" <<EOF
...
docker build -t latticed-proxy proxy/ -q 2>&1

echo "[setup] Starting proxy container..."
docker run -d --name "$CONTAINER_NAME" \
    -p "${PROXY_PORT}:${PROXY_PORT}" \
    -v "${CADDYFILE}:/etc/caddy/Caddyfile:ro" \
    latticed-proxy > /dev/null 2>&1

echo "[setup] Waiting for proxy to be ready..."
sleep 3

CA_CERT=$(mktemp /tmp/lattice-ca-XXXXXX.crt)
...
HEADER_FILE=$(mktemp /tmp/lattice-headers-XXXXXX)
