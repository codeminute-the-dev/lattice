"""
Lattice quantization config for vLLM.
...
from .vllm_scheme import LatticeScheme

_LOGGER = get_logger("vllm.lattice_miner")


class LatticeConfig(CompressedTensorsConfig):
    """
    Lattice quantization config extending CompressedTensorsConfig.
    ...
    @override
    @classmethod
    def from_config(cls, config: dict[str, Any]) -> "LatticeConfig":
        """Create LatticeConfig from config dict."""
    ...
    def get_quant_method(
        ...
        """Route mixed int7-gate/up + fp8-block-down FusedMoE layers to LatticeMoE."""
        from vllm.model_executor.layers.fused_moe import FusedMoE

        from .lattice_moe_method import LatticeMoEMethod
        ...
        if isinstance(layer, FusedMoE):
            ...
            if self._is_mining_layer(gate_weight, gate_input) and self._is_fp8_block_layer(
                ...
                _LOGGER.debug(f"Lattice MoE (int7 gate/up + fp8 block down) detected for {prefix}")
                return LatticeMoEMethod(layer.moe_config, down_weight, down_input)
    ...
    def _get_scheme_from_parts(
        ...
        Checks for:
        1. Mining layer (7-bit) -> LatticeScheme(mining_enabled=True)
        2. Non-mining layer (8-bit) -> LatticeScheme(mining_enabled=False)
