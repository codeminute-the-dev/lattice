...
class LatticeMoEMethod(FusedMoEMethodBase):
    def __init__(
        self,
        ...
        if not block_shape:
            raise ValueError(
                "LatticeMoE down projection requires a fp8 weight block_structure "
        ...
        if group_size is None:
            raise ValueError(
                "LatticeMoE down projection requires an activation group_size in "
    ...
    def _warn_backend_ignored(
        ...
        _LOGGER.warning(
            "Requested MoE backend '%s' is ignored for LatticeMoE; the down "
    ...
    def _setup_kernel(self, layer: torch.nn.Module) -> None:
        ...
        from .lattice_moe_experts import LatticeMoEExperts

        try:
            self.moe_quant_config = self.get_fused_moe_quant_config(layer)
            prepare_finalize = MoEPrepareAndFinalizeNoDPEPModular()
            experts = LatticeMoEExperts(
                moe_config=self.moe,
            ...
            _LOGGER.info("Using LatticeMoEExperts for MoE layer")
        except Exception:
            # Leave moe_kernel unset; layer may initialise via maybe_init_modular_kernel.
            _LOGGER.exception("Failed to set up LatticeMoE kernel; will rely on lazy init")
    ...
    def apply(
        ...
        assert self.moe_kernel is not None, (
            "LatticeMoEMethod.apply called before moe_kernel was initialised"
        )
    ...
    def select_gemm_impl(
        ...
        if prepare_finalize.activation_format == FusedMoEActivationFormat.BatchedExperts:
            raise NotImplementedError(
                "BatchedExperts activation format is not supported for LatticeMoE"
            )
        from .lattice_moe_experts import LatticeMoEExperts

        return LatticeMoEExperts(
            moe_config=self.moe,
            quant_config=self.moe_quant_config,
            layer=layer,
            w2_block_shape=self._w2_block_shape(),
