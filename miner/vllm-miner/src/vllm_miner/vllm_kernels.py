...
Both modes use lattice GEMM kernels and support smooth_quant_scale.
"""
...
class LatticeKernel(Int8ScaledMMLinearKernel):
    """
    Unified kernel supporting mining and non-mining modes.

    Both modes use lattice GEMM kernels.
    ...
    def get_min_capability(cls) -> int:
        # Lattice GEMM kernels require Hopper or newer
        return 9
    ...
    def can_implement(cls, c: Int8ScaledMMLinearLayerConfig) -> tuple[bool, str | None]:
        if not current_platform.is_cuda():
            return False, "LatticeKernel requires running on CUDA."
        return True, None

    @override
    @classmethod
    def is_supported(cls, compute_capability: int | None = None) -> tuple[bool, str | None]:
        """Check if LatticeKernel is supported on the current hardware."""
        if compute_capability is None:
            if not current_platform.is_cuda():
                return False, "LatticeKernel requires CUDA."
            compute_capability = current_platform.get_device_capability()[0]

        if compute_capability < cls.get_min_capability():
            return (
                False,
                f"LatticeKernel requires compute capability >= {cls.get_min_capability()}, got {compute_capability}.",
            )
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        """
        # WEIGHT
        # Lattice GEMM kernels expect non-transposed weights
        ...
        if not self.config.input_symmetric:
            raise NotImplementedError("Only symmetric quantization is supported for lattice GEMM")
    ...
    def apply_weights(
        ...
        """
        Applies quantized weights to input tensor using lattice GEMM.
