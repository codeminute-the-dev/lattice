...
from lattice_gemm import get_host_signal_sync_size
...
# MoE noise kernels require at least Hopper (matches dense LatticeKernel min capability).
...
class LatticeMoEExperts(mk.FusedMoEExpertsModular):
    """Lattice MoE experts: per-expert noisy GEMM1 (mining) + vanilla GEMM2."""
    ...
    def apply(
        ...
        ) and lattice_config.should_use_noisy_gemm(num_tokens, N, K)
    ...
    def _apply_per_expert_gemm1(
        ...
        *,
        ...
        tile_m = lattice_config.settings.tile_size_m
        tile_n = lattice_config.settings.tile_size_n
        tile_k = lattice_config.settings.tile_size_k
