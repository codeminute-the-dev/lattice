...
GATEWAY_SOCKET="${MINER_RPC_SOCKET_PATH:-/tmp/latticegw.sock}"
echo "Waiting for lattice-gateway socket at $GATEWAY_SOCKET ..."
for _ in $(seq 1 30); do
    if [ -S "$GATEWAY_SOCKET" ]; then
        break
    fi
    if ! kill -0 "$LATTICE_PID" 2>/dev/null; then
        echo "lattice-gateway exited before becoming ready" >&2
        exit 1
    fi
    sleep 1
done
if [ ! -S "$GATEWAY_SOCKET" ]; then
    echo "Timed out waiting for lattice-gateway socket at $GATEWAY_SOCKET" >&2
    exit 1
fi
