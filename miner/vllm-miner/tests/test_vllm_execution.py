#!/usr/bin/env python3
"""
Tests for vLLM execution with LatticeMiner mining control.

Parametrized over both dense (Llama-8B) and MoE (Qwen3-30B-A3B) models. Verifies that:
1. vLLM can be initialized with Lattice plugin support
...
LLAMA_8B = ModelTestConfig(
    id="llama_8b",
    model="lattice-ai/Llama-3.1-8B-Instruct-lattice",
    prompt=(
    ...
    plugin_log_msg="Using LatticeKernel (mining_enabled=True) for mining layer",
)

QWEN3_MOE = ModelTestConfig(
    id="qwen3_moe",
    model="lattice-ai/Qwen3-30B-A3B-Instruct-2507-lattice",
    prompt=(
        "<|im_start|>system\n"
        "You are a French translator. Reply with ONLY the French translation of the user's "
    ...
    plugin_log_msg="Using LatticeMoEExperts for MoE layer",
)
...
def _reference_key(model_config: ModelTestConfig, suffix: str) -> str:
    """Key into ``reference_outputs.json`` (Lattice only targets H100, so no GPU-family bucket)."""
...
def manage_mining_state(lattice_gateway_process):
...
def lattice_gateway_process(sample_block_template, mining_address):  # noqa: C901
    """
    Start lattice-gateway programmatically once per test session.
    ...
    # Set mining address environment variable required by LatticeConfig
    os.environ["LATTICED_MINING_ADDRESS"] = mining_address

    # Use default socket path from lattice-gateway config
    socket_path = "/tmp/latticegw.sock"
    ...
    async def start_gateway_with_mock_template():
        ...
        gateway_instance = LatticeGateway(debug_mode=True)
...
def test_plugin_loaded(model_config, get_llm_instance, is_mining_enabled):
    """The Lattice plugin's per-layer log message appears during vLLM init."""
