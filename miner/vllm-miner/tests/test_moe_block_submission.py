...
from lattice_gateway.comm.mining_configuration import MoEConfig
...
from vllm_miner.lattice_moe_method import LatticeMoEMethod
...
@pytest.fixture
def lattice_moe_layer():
    """Build a LatticeMoEMethod + mock layer using the production weight-loading flow."""
    ...
    moe_method = LatticeMoEMethod(moe_config, _DOWN_WEIGHT_QUANT, _DOWN_INPUT_QUANT)
...
def test_block_found_and_proof_verifies(lattice_moe_layer, get_mining_job, async_manager):
    """Full MoE flow: apply() -> PoW hit -> create_proof -> submit (dummy gateway)."""
    moe_method, layer, tensors = lattice_moe_layer
...
class TestBlockSubmissionIntegration:
    """End-to-end MoE flow against a real Lattice node (requires a running node)."""

    @pytest.mark.integration
    def test_block_submission(self, real_gateway, async_manager_real, lattice_moe_layer):
        """Mine via LatticeMoEMethod.apply, generate the ZK proof in the gateway, and
        submit to the real node:

        1. LatticeMoEMethod.apply mines a block (per-expert noisy_gemm + PoW)
        ...
        logger.info("SUCCESS! MoE block was ACCEPTED by the Lattice node!")
