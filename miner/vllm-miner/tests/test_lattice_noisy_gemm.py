...
from lattice_gemm import get_host_signal_header_size, get_host_signal_sync_size, noisy_gemm
...
def test_lattice_gemm_noisy_with_block_found(
    ...
    with patch.object(get_async_manager(), "_client") as mock_mining_client:
        ...
        _ = lattice_gemm_noisy(A, B, scale_a.squeeze(), scale_b.squeeze(), out_dtype)

        get_async_manager().wait_until_done_submitting_blocks()
        assert get_async_manager().blocks_submitted == 1


def test_lattice_gemm_noisy_with_zero_noise(make_random_test_matrices):
    """Test lattice_gemm_noisy with zero noise (should behave like vanilla GEMM)."""
    ...
    output_with_zero_noise, ApEA, BpEB = lattice_gemm_noisy_with_zero_noise(
...
def test_lattice_gemm_noisy_with_real_noise(make_random_test_matrices):
    """Test lattice_gemm_noisy with real noise generation."""
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
    ...
    output_with_zero_noise, ApEA_zero, BpEB_zero = lattice_gemm_noisy_with_zero_noise(
        a, b, scale_a.squeeze(), scale_b.squeeze(), out_dtype
    )

    # Test with real noise using the actual lattice_gemm_noisy function
    print("Testing with real noise...")
    output_with_real_noise = lattice_gemm_noisy(a, b, scale_a.squeeze(), scale_b.squeeze(), out_dtype)
    ...
    if output_with_real_noise.std().item() < 1e-6:
        ...
        print("This indicates a bug in the lattice_gemm_noisy function")
    ...
    vanilla_output = lattice_gemm_vanilla(a, b, scale_a.squeeze(), scale_b.squeeze(), out_dtype)
    ...
    outputs = []
    for _i in range(5):
        output = lattice_gemm_noisy(a, b, scale_a.squeeze(), scale_b.squeeze(), out_dtype)
...
def lattice_gemm_noisy_with_zero_noise(a, b, scale_a, scale_b, out_dtype):
    """
    Modified version of lattice_gemm_noisy that uses zero noise matrices.
    ...
    # First, call noise_B
    # torch.ops.lattice_gemm.noise_B(
...
@pytest.mark.flaky(reruns=3)
def test_lattice_gemm_noisy_with_controlled_noise(make_random_test_matrices):
    """Test that lattice_gemm_noisy produces consistent results with controlled noise."""
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
