...
from lattice_gemm.test_components import inner_hash as inner_hash_cuda
...
def _offsets_hash_cuda(routing_offsets: list[int], key: bytes) -> bytes:
    ...
    scratchpad = torch.empty(
        lattice_gemm.get_required_scratchpad_bytes(offsets_bytes.numel()),
        dtype=torch.uint8,
        device="cuda",
    )
    lattice_gemm.tensor_hash(offsets_bytes.reshape(1, -1), key_tensor, offsets_hash, scratchpad)
...
class TestMatrixMerkleTreeVsTensorHash:
    ...
    def test_matrix_merkle_tree_vs_tensor_hash(self, test_noise_seed_A, m, n):
        ...
        tensor_hash_scratchpad = torch.empty(
            lattice_gemm.get_required_scratchpad_bytes(matrix_uint8.numel()),
            device="cuda",
            dtype=torch.uint8,
        )
        lattice_gemm.tensor_hash(matrix_uint8, key_tensor, cuda_result, tensor_hash_scratchpad)
        torch.cuda.synchronize()
    ...
    def test_commitment_hash_reference_vs_cuda(self, test_noise_seed_A, salted_dims):
        ...
        lattice_gemm.commitment_hash_from_merkle_roots(
