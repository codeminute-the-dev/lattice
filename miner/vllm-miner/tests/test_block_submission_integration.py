"""
End-to-end integration tests with a real Lattice node.

Uses configuration from lattice_gateway/config.yaml which contains
the Lattice node connection settings.
"""
...
from vllm_miner.gemm_operators import lattice_gemm_noisy
...
def test_block_submission(real_gateway, async_manager_real, make_random_test_matrices):
    """
    Test complete end-to-end flow with a real Lattice node:
    1. Miner connects to gateway via lattice_gemm_noisy
    2. Gateway fetches block template from real node
    3. Miner mines and finds a valid block
    4. Gateway generates ZK proof
    5. Block is submitted to real Lattice node
    ...
    logger.info("Starting mining loop via lattice_gemm_noisy, waiting for block...")
    ...
    while async_manager.blocks_submitted == 0:
        ...
        _ = lattice_gemm_noisy(A, B, scale_a.squeeze(), scale_b.squeeze(), out_dtype, bias)
        matmul_count += 1
    ...
    logger.info("SUCCESS! Block was ACCEPTED by the Lattice node!")
