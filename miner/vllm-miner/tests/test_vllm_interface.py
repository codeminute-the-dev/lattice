"""
Tests for LatticeKernel vLLM interface.
...
from vllm_miner import LatticeKernel
from vllm_miner.config import config as lattice_config
from vllm_miner.quantization_operators import quant_7bit, quant_8bit
from vllm_miner.vllm_config import LatticeConfig
from vllm_miner.vllm_scheme import LatticeScheme
...
def test_apply_weights_mining_enabled(m, n, k, async_manager):
    """Test LatticeKernel with mining_enabled=True (uses int7 + vanilla/noisy GEMM)."""
...
def test_apply_weights_mining_disabled(m, n, k, async_manager):
    """Test LatticeKernel with mining_enabled=False (uses int8 + vanilla GEMM)."""
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
...
def test_kernel_default_is_mining_enabled():
    """Test that LatticeKernel defaults to mining_enabled=True."""
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
...
def test_apply_weights_with_smooth_quant_scale(mining_enabled, async_manager):
    ...
    kernel_no_smooth = LatticeKernel(
        DEFAULT_QUANT_CONFIG,
        DEFAULT_LAYER_PARAM_NAMES,
        mining_enabled=mining_enabled,
    )
...
class TestNoisyGemmSelectionThresholds:
    ...
    def test_should_use_noisy_gemm_all_above_threshold(self):
        ...
        assert lattice_config.should_use_noisy_gemm(1024, 1024, 1024) is True
        assert lattice_config.should_use_noisy_gemm(2048, 2048, 2048) is True
        assert lattice_config.should_use_noisy_gemm(4096, 8192, 1024) is True

    def test_should_use_noisy_gemm_below_m_threshold(self):
        """Test that vanilla GEMM is selected when m < threshold."""
        # m below threshold
        assert lattice_config.should_use_noisy_gemm(512, 1024, 1024) is False
        assert lattice_config.should_use_noisy_gemm(1, 2048, 2048) is False

    def test_should_use_noisy_gemm_below_n_threshold(self):
        """Test that vanilla GEMM is selected when n < threshold."""
        # n below threshold
        assert lattice_config.should_use_noisy_gemm(1024, 128, 1024) is False
        assert lattice_config.should_use_noisy_gemm(2048, 1, 2048) is False

    def test_should_use_noisy_gemm_below_k_threshold(self):
        """Test that vanilla GEMM is selected when k < threshold."""
        # k below threshold
        assert lattice_config.should_use_noisy_gemm(1024, 1024, 512) is False
        assert lattice_config.should_use_noisy_gemm(2048, 2048, 1) is False
    ...
    def test_should_use_noisy_gemm_boundary_cases(self):
        ...
        assert lattice_config.should_use_noisy_gemm(1024, 1024, 1024) is True
        assert lattice_config.should_use_noisy_gemm(1024, 256, 1024) is True

        # Just below threshold - should use vanilla GEMM
        assert lattice_config.should_use_noisy_gemm(1023, 1024, 1024) is False
        assert lattice_config.should_use_noisy_gemm(1024, 255, 1024) is False
        assert lattice_config.should_use_noisy_gemm(1024, 1024, 1023) is False
...
class TestLatticeConfig:
    """Tests for LatticeConfig layer detection and scheme selection."""
    ...
    def test_is_mining_layer_7bit(self):
        ...
        assert LatticeConfig._is_mining_layer(weight_quant, input_quant) is True
    ...
    def test_is_mining_layer_8bit_not_mining(self):
        ...
        assert LatticeConfig._is_mining_layer(weight_quant, input_quant) is False
    ...
    def test_is_non_mining_layer_8bit(self):
        ...
        assert LatticeConfig._is_non_mining_layer(weight_quant, input_quant) is True
    ...
    def test_is_non_mining_layer_7bit_not_non_mining(self):
        ...
        assert LatticeConfig._is_non_mining_layer(weight_quant, input_quant) is False

    def test_get_scheme_returns_mining_scheme_for_7bit(self):
        """Test that _get_scheme_from_parts returns LatticeScheme with mining_enabled=True for 7-bit."""
        # Create a minimal LatticeConfig with all required arguments
        cfg = LatticeConfig(
            target_scheme_map={},
            ignore=[],
            quant_format=None,
        ...
        assert isinstance(scheme, LatticeScheme)
        assert scheme.mining_enabled is True

    def test_get_scheme_returns_non_mining_scheme_for_8bit(self):
        """Test that _get_scheme_from_parts returns LatticeScheme with mining_enabled=False for 8-bit."""
        # Create a minimal LatticeConfig with all required arguments
        cfg = LatticeConfig(
            target_scheme_map={},
            ignore=[],
    ...
    def test_none_quant_args_returns_false(self):
        ...
        assert LatticeConfig._is_mining_layer(None, None) is False
        assert LatticeConfig._is_non_mining_layer(None, None) is False

        weight_quant = self._make_quant_args(7, strategy="tensor", dynamic=False)
        assert LatticeConfig._is_mining_layer(weight_quant, None) is False
        assert LatticeConfig._is_mining_layer(None, weight_quant) is False
...
class TestLatticeScheme:
    """Tests for LatticeScheme."""

    @pytest.mark.parametrize("mining_enabled", [True, False])
    def test_scheme_creates_kernel_with_correct_mode(self, mock_vllm_distributed, mining_enabled):
        """Test that LatticeScheme creates kernel with correct mining_enabled setting."""
