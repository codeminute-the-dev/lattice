...
from lattice_mining import PlainProof, check_cert_version_eligible

from lattice_gateway.comm.dataclasses import BlockTemplate
from lattice_gateway.lattice_client import LatticeNodeClient
from lattice_gateway.proof_generator import ProofGenerator
...
class SubmissionService:
    """
    Handles block submissions from miners to the Lattice node.
    Receives PlainProof from miners and generates complete blocks.
    """

    def __init__(self, lattice_client: LatticeNodeClient, debug_mode: bool = False):
    ...
    async def submit_plain_proof(
        ...
        async with self.submission_lock:
            try:
                ...
                # Submit to the Lattice node
                self.submitted_blocks += 1
                result = await self.lattice_client.submit_block(block.serialize().hex())
