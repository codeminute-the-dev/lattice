...
@pytest.fixture
def mock_lattice_client():
    """Create a mock LatticeNodeClient."""
    client = AsyncMock()
...
@pytest.fixture
async def scheduler(mock_lattice_client, mock_work_cache, sample_lattice_config):
    """Create a TemplateScheduler instance for testing."""
    scheduler_instance = TemplateScheduler(mock_lattice_client, mock_work_cache, sample_lattice_config)
...
class TestSchedulerStartStop:
    ...
    @pytest.mark.asyncio
    async def test_start_scheduler(self, scheduler, mock_lattice_client, sample_block_template):
        """Test starting the scheduler."""
        mock_lattice_client.get_block_template.return_value = sample_block_template

        await scheduler.start()

        assert scheduler.running is True
        assert scheduler.refresh_task is not None
        mock_lattice_client.get_block_template.assert_called_once()

    @pytest.mark.asyncio
    async def test_start_scheduler_already_running(
        self, scheduler, mock_lattice_client, sample_block_template
    ):
        ...
        # Start once
        await scheduler.start()
        call_count_after_first_start = mock_lattice_client.get_block_template.call_count

        # Start again
        await scheduler.start()

        # Should not call get_block_template again
        assert mock_lattice_client.get_block_template.call_count == call_count_after_first_start

    @pytest.mark.asyncio
    async def test_stop_scheduler(self, scheduler, mock_lattice_client, sample_block_template):
...
class TestTemplateRefresh:
    ...
    async def test_refresh_template_success(
        self, scheduler, mock_lattice_client, mock_work_cache, sample_block_template
    ):
    ...
    @pytest.mark.asyncio
    async def test_refresh_template_lattice_client_error(
        self, scheduler, mock_lattice_client, mock_work_cache
    ):
        """Test template refresh when lattice client fails."""
        mock_lattice_client.get_block_template.side_effect = ConnectionError("Connection failed")
...
class TestPeriodicRefresh:
    ...
    async def test_periodic_refresh_handles_exceptions(
        ...
        mock_lattice_client.get_block_template.side_effect = [
        ...
        assert mock_lattice_client.get_block_template.call_count >= 3  # 1 from start + 2 manual calls
...
class TestSchedulerIntegration:
    ...
    async def test_full_scheduler_lifecycle(
        ...
        mock_lattice_client.get_block_template.assert_called()
    ...
    async def test_scheduler_with_different_intervals(
        ...
        short_scheduler = TemplateScheduler(mock_lattice_client, mock_work_cache, short_config)
        assert short_scheduler.refresh_interval == 1

        # Test with longer interval
        long_config = LatticeConfig(
            mining_address=mining_address,
            rpc_url="https://127.0.0.1:18334",
            rpc_user="test_user",
            rpc_password="test_password",
        ...
        long_scheduler = TemplateScheduler(mock_lattice_client, mock_work_cache, long_config)
    ...
    async def test_scheduler_error_recovery(
        ...
        assert result is True
        assert mock_lattice_client.get_block_template.call_count == 3
