...
@pytest.fixture
def submission_service(mock_lattice_client):
    """Create a SubmissionService instance for testing."""
    return SubmissionService(mock_lattice_client)
...
class TestBlockSubmission:
    ...
    async def test_submit_block_accepted(
        ...
        sample_lattice_block,
    ):
        """Test successful block submission that gets accepted."""
        mock_lattice_client.submit_block.return_value = "accepted"

        with patch(
            "lattice_gateway.proof_generator.ProofGenerator.generate_block",
            return_value=sample_lattice_block,
        ):
        ...
        mock_lattice_client.submit_block.assert_called_once_with(sample_lattice_block.serialize().hex())
    ...
    async def test_submit_block_rejected(
        ...
        mock_lattice_client.submit_block.return_value = "rejected: invalid proof"
    ...
    @pytest.mark.asyncio
    async def test_submit_block_lattice_client_error(
        self,
        submission_service,
        mock_lattice_client,
        sample_plain_proof,
        ...
        """Test block submission when lattice client raises an exception."""
        mock_lattice_client.submit_block.side_effect = ConnectionError("Connection failed")
    ...
    async def test_submit_block_thread_safety(
        ...
        mock_lattice_client.submit_block.side_effect = mock_submit_with_delay

        with patch(
            "lattice_gateway.proof_generator.ProofGenerator.generate_block",
            side_effect=[sample_lattice_block, sample_lattice_block],
        ):
...
class TestCrossoverEnforcement:
    ...
    async def test_moe_proof_before_crossover_rejected(
        ...
        mock_lattice_client.submit_block.assert_not_called()
...
class TestSubmissionServiceIntegration:
    ...
    async def test_multiple_submissions_workflow(
        ...
        mock_lattice_client.submit_block.side_effect = [
            "accepted",
            "rejected: duplicate",
            "accepted",
