...
from lattice_gemm.testing import GEMMParam, GemmTensorGenerator

DISABLE_R32 = os.getenv("LATTICE_GEMM_DISABLE_R32", "TRUE") == "TRUE"
DISABLE_R64 = os.getenv("LATTICE_GEMM_DISABLE_R64", "FALSE") == "TRUE"
DISABLE_R128 = os.getenv("LATTICE_GEMM_DISABLE_R128", "FALSE") == "TRUE"
Rs = (
    []
...
DISABLE_DEBUG_MODE = os.getenv("LATTICE_GEMM_DISABLE_DEBUG_MODE", "FALSE") == "TRUE"

# Run tests against default compiled kernels
from lattice_gemm_build_utils.kernel_configs.default_compiled_kernels import (  # noqa: E402
    KERNEL_CONFIGS,
)
...
# Base Lattice GEMM test class with util functions. Because of parameterize, it's difficult to
#  select individual test functions. So the tests are grouped into classes. Tests should
#  extend this class.
class TestLatticeGEMMBase:
    # Always reset seed
    ...
    #### Wrapping lattice GEMM interface ####
...
class TestDenoiseConverter(TestLatticeGEMMBase):
    ...
    def test_denoise_converter_opcheck(self, R, convert_denoise):
        ...
        torch.library.opcheck(
            torch.ops.lattice_gemm.denoise_converter.default, args=(), kwargs=kwargs
        )
...
class TestNoiseA(TestLatticeGEMMBase):
    ...
    def test_noise_a_opcheck(
        ...
        torch.library.opcheck(torch.ops.lattice_gemm.noise_A.default, args=(), kwargs=kwargs)
    ...
    def test_int7_test_torch_noise_a(
        ...
        def op_noise_a_simplified(noise_a_args):
            torch.ops.lattice_gemm.noise_A(**noise_a_args)
...
class TestNoiseB(TestLatticeGEMMBase):
    ...
    def test_noise_b_opcheck(self, k_blocks_per_split_noising_B, noising_b_config):
        ...
        torch.library.opcheck(torch.ops.lattice_gemm.noise_B.default, args=(), kwargs=kwargs)
    ...
    def test_int7_test_torch_noise_b(self, m, n, k, noising_b_config):
        ...
        def op_noise_b_simplified(noise_b_args):
            torch.ops.lattice_gemm.noise_B(**noise_b_args)
...
class TestNoisyGEMM(TestLatticeGEMMBase):
    @pytest.mark.skip
    ...
    def test_noisy_gemm_opcheck(self, matmul_config):
        ...
        torch.library.opcheck(torch.ops.lattice_gemm.noisy_gemm.default, args=(), kwargs=gemm_kwargs)
    ...
    def test_int7_noisy_torch_gemm(self, m, n, k, noising_dtype, matmul_config):
        ...
        def op_noisy_gemm_simplified(gemm_kwargs):
            torch.ops.lattice_gemm.noisy_gemm(**gemm_kwargs)
    ...
    def test_int7_noisy_torch_gemm_separate_functions(self, m, n, k, noising_dtype, matmul_config):
        ...
        def op_gemm_simplified(noise_a_kwargs, noise_b_kwargs, gemm_kwargs):
            torch.ops.lattice_gemm.noise_A(**noise_a_kwargs)
            torch.ops.lattice_gemm.noise_B(**noise_b_kwargs)
            torch.ops.lattice_gemm.noisy_gemm(**gemm_kwargs)
...
class TestGEMM(TestLatticeGEMMBase):
    @pytest.mark.parametrize("matmul_config", matmul_kernels)
    def test_gemm_opcheck(self, matmul_config):
        ...
        torch.library.opcheck(torch.ops.lattice_gemm.gemm.default, args=(), kwargs=gemm_kwargs)
    ...
    def test_noiseless_int7_torch_gemm(self, m, n, k, matmul_config):
        ...
        def op_gemm_simplified(gemm_kwargs):
            torch.ops.lattice_gemm.gemm(**gemm_kwargs)
...
class TestSwizzleNoisyGEMM(TestLatticeGEMMBase):
...
class TestSwizzleGEMM(TestLatticeGEMMBase):
    @pytest.mark.parametrize("m", [512, 513])
    @pytest.mark.parametrize("n", [8192, 8200, 28672])
    @pytest.mark.parametrize("k", [256, 8192])
    @pytest.mark.parametrize("matmul_config", matmul_kernels)
...
class TestSkipReductionNoisyGEMM(TestLatticeGEMMBase):
    @pytest.mark.parametrize("m", [128, 1024, 1025, 8192])
    @pytest.mark.parametrize("n", [128, 1024, 1032, 8192])
...
class TestInnerHashCounting(TestLatticeGEMMBase):
    ...
    def test_inner_hash_count(self, m, n, k, matmul_kernel_config):
        ...
        torch.ops.lattice_gemm.noisy_gemm(
            A=tg.A,
            B=tg.B,
            EAL=tg.EAL,
            EAL_fp16=tg.EAL_fp16,
