"""
Test Components Module for lattice_gemm

This module provides Python bindings for testing individual components
of the lattice_gemm library, including the inner_hash function.
"""
...
def inner_hash(input_buffer: torch.Tensor, iterations: int = 1) -> torch.Tensor:
    ...
    """
    return torch.ops.lattice_gemm.inner_hash(input_buffer, iterations)
...
def tensor_hash(
    ...
    """
    return torch.ops.lattice_gemm.tensor_hash(data, key, out, roots)  # type: ignore
...
def commitment_hash_from_merkle_roots(
    ...
    *,
    ...
    return torch.ops.lattice_gemm.commitment_hash_from_merkle_roots(
        A_merkle_root,
        B_merkle_root,
        key,
        commitment_hash_A,
