...
void run_lattice_gemm(LatticeAPIParams const& params, cudaStream_t stream = 0) {
  ...
  using KTraits =
      lattice::KernelTraits<ElementIn, ElementOut, ElementDenoise, ElementScale,
                          TileShape_MNKR, Is_Even_M, Is_Even_N, cM, cN,
                          SkipReduction, SkipDenoising, KStages, EnableDebug>;
  using CollectiveEpilogue = lattice::CollectiveEpilogue<KTraits>;
  ...
  using CollectiveMainloop = lattice::CollectiveMainloop<KTraits>;

  using ClusterShape = typename KTraits::ClusterShape_MNK;
  // currently only supporting SingleTileScheduler
  using Scheduler = lattice::SingleTileScheduler;
  ...
  void* kernel = (void*)lattice::hopper_gemm_ws<KTraits, Scheduler>;
