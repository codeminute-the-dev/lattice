...
#include "lattice_gemm_host.h"
#include "lattice_noisingA_host.h"
#include "lattice_noisingB_host.h"
#include "static_switch.h"

template <class ElementDenoise_AxEBL, int R, int bM_noising, int bK_noising,
          int kStages>
void run_lattice_noising_A_(LatticeAPIParams& params, cudaStream_t stream = 0) {
...
void run_lattice_gemm_(LatticeAPIParams& params, cudaStream_t stream = 0) {
  using namespace cute;
  ...
  BOOL_SWITCH(
      ...
      BOOL_SWITCH(
          ...
          run_lattice_gemm<ElementOut, TileShape_MNKR, kStages, cM, cN, IsEvenM,
