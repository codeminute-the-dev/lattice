...
#include "lattice_gemm_constants.hpp"
#include "lattice_gemm_decl.h"
...
void noise_A(at::Tensor& A,                          // m x k
  ...
  NOISING_A_CONFIG_SWITCH(
      ...
      kernel_found = true;
      run_lattice_noising_A_<ElementDenoise_AxEBL, R_, bM_, bK_, stages_>(
          params, stream););
...
void noise_B(at::Tensor& B,                          // n x k
  ...
  NOISING_B_CONFIG_SWITCH(
      ...
      kernel_found = true;
      run_lattice_noising_B_<ElementDenoise_EARxBpEB, R_, bN_, bK_, stages_>(
          params, stream););
...
void gemm(at::Tensor& A,         // m x k
  ...
  for (int const R : {64, 128}) {
    MATMUL_CONFIG_SWITCH(
        ...
        run_lattice_gemm_<ElementOut, R_, bM_, bN_, bK_, stages_, cM_, cN_,
          ...
          SKIP_DENOISING_SWITCH(
              ...
              if (run_noising_a) {
                NOISING_A_CONFIG_SWITCH(
                    ...
                    run_lattice_noising_A_<ElementDenoise_AxEBL, R_, bM_, bK_,
              ...
              if (run_noising_b) {
                NOISING_B_CONFIG_SWITCH(
                    ...
                    run_lattice_noising_B_<ElementDenoise_EARxBpEB, R_, bN_, bK_,
                                         stages_>(params, stream););
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.doc() = "Lattice GEMM with noising/denoising and PoW extraction";
  ...
  m.attr("kEALScaleFactorDenoise") = lattice::kEALScaleFactorDenoise;
  m.attr("kEBRScaleFactorDenoise") = lattice::kEBRScaleFactorDenoise;
...
// For torch ops. Using the same default values as ones used in the lattice_gemm_interface.py
TORCH_LIBRARY(lattice_gemm, m) {
  m.def(
      "noisy_gemm("
      "    Tensor A, "
      "    Tensor B, "
...
TORCH_LIBRARY_IMPL(lattice_gemm, CUDA, m) {
