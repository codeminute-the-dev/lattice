# Install latticed, latctl, latwallet, and latwalletcli from Lattice GitHub Releases (Windows).
...
<#
.SYNOPSIS
  Install Lattice release binaries and mainnet configs on Windows.

.PARAMETER Version
  Release tag to install (default: latest stable), e.g. v1.1.5

.PARAMETER BinDir
  Install directory (default: $env:LOCALAPPDATA\Lattice\bin)
...
$OptionalBinaries = @('latwalletcli')
$AllowedArchiveBins = $Binaries + $OptionalBinaries + 'latmon'
$TempDir = $null
$RpcUser = $null
$RpcPass = $null

function Write-InstallLog([string]$Message) {
	Write-Host "lattice-install: $Message"
}
...
function Test-ConfigHasSecrets([string]$Name, [string]$File) {
	$keys = switch ($Name) {
		'latwallet' { @('username', 'password') }
		{ $_ -in 'latticed', 'latctl' } { @('rpcuser', 'rpcpass') }
...
username=$RpcUser
password=$RpcPass
latticedusername=$RpcUser
latticedpassword=$RpcPass
...
function Initialize-RpcCredentials {
	foreach ($probe in @(
			@{ Path = (Get-AppConfigPath 'latticed'); User = 'rpcuser'; Pass = 'rpcpass' }
			@{ Path = (Get-AppConfigPath 'latwallet'); User = 'username'; Pass = 'password' }
			@{ Path = (Get-AppConfigPath 'latctl'); User = 'rpcuser'; Pass = 'rpcpass' }
		)) {
...
try {
	if ($Help) {
		@'
Install Lattice release binaries (latticed, latctl, latwallet) and mainnet configs.
...
'@
	...
	if (-not $BinDir) { $BinDir = Join-Path (Get-LocalAppData) 'Lattice\bin' }
	try {
		New-Item -ItemType Directory -Path $BinDir -Force | Out-Null
		$probe = Join-Path $BinDir ".lattice-install-write-test.$PID"
		[IO.File]::WriteAllText($probe, 'ok')
		Remove-Item -LiteralPath $probe -Force
	} catch {
		throw "install directory is not writable: $BinDir`ntry: .\install.ps1 -BinDir `"$env:LOCALAPPDATA\Lattice\bin`""
	}
	...
	$archiveName = "lattice-windows-$arch-$Version.zip"
	$assetBase = "https://github.com/$Repo/releases/download/$Version"

	$TempDir = Join-Path ([IO.Path]::GetTempPath()) ("lattice-install." + [guid]::NewGuid().ToString('N'))
	...
	$created = $false
	foreach ($name in @('latticed', 'latwallet', 'latctl')) {
	...
	$configLines = (@('latticed', 'latwallet', 'latctl') | ForEach-Object { "  $(Get-AppConfigPath $_)" }) -join "`n"
	$latwalletcliLine = ''
	if (Test-Path -LiteralPath (Join-Path $BinDir 'latwalletcli.exe')) {
		$latwalletcliLine = "`n  latwalletcli              # interactive wallet UI"
	}
	@"
...
Next steps (no -u/-P/-C needed):
  ...
  latctl getinfo
  latctl --wallet getinfo$latwalletcliLine
"@ | Write-Host
	...
	[Console]::Error.WriteLine("lattice-install: $($_.Exception.Message)")
	exit 1
} finally {
