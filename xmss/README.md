...
Go bindings for the XMSS (eXtended Merkle Signature Scheme) post-quantum signature library used by the Lattice network.

XMSS is a hash-based signature scheme that is secure against quantum adversaries. Lattice uses it to implement the `OP_CHECKXMSSSIG` opcode, providing a post-quantum spend path for Taproot addresses.
