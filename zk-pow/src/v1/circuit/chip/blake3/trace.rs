use crate::v1::{
    ...
    circuit::{
        ...
        lattice_layout::{BYTES_PER_GOLDILOCKS, lattice_columns},
...
pub fn fill_blake_data<F: RichField + Extendable<D>, const D: usize>(
    ...
    for row_idx in 0..num_rows {
        ...
        let cv_in: [u32; 8] = blake
            ...
            .map(|i| read_from_trace(&trace[i], lattice_columns::CV_OUT))
            .unwrap_or([0; 8]);
        write_to_trace(&mut trace[row_idx], lattice_columns::CV_IN, &cv_in);
        ...
        write_to_trace(&mut trace[row_idx], lattice_columns::BLAKE3_CV, &blake3_cv);
        ...
        if blake.round_idx == 8 {
            ...
            let cv: [u32; 8] = read_from_trace(&trace[first], lattice_columns::BLAKE3_CV);
            ...
            write_to_trace(&mut trace[row_idx], lattice_columns::CV_OUT, &cv_out);

            write_to_trace(&mut trace[row_idx], lattice_columns::BLAKE3_MSG_BUFFER, &buffer);
            let mut msg = blake3_inverse_permute_msg(&buffer);
            write_to_trace(&mut trace[row_idx], lattice_columns::BLAKE3_MSG, &msg);
            for r in (row_idx.saturating_sub(7)..row_idx).rev() {
                msg = blake3_inverse_permute_msg(&msg);
                write_to_trace(&mut trace[r], lattice_columns::BLAKE3_MSG, &msg);
            }
            for r in row_idx.saturating_sub(7)..row_idx {
                let len = 2 * (r + 8 - row_idx);
                write_to_trace(&mut trace[r], lattice_columns::BLAKE3_MSG_BUFFER + 16 - len, &buffer[..len]);
            }
            let mut fwd = buffer;
            for off in 1..=7 {
                ...
                write_to_trace(
                    ...
                    lattice_columns::BLAKE3_MSG_BUFFER,
                    &fwd[..16 - 2 * off],
                );
            }
        }
    ...
    blocks.par_iter().for_each(|&(first, pivot, params)| {
        ...
        for row_idx in first..=pivot {
            let row = unsafe { &mut *(trace.as_ptr() as *mut [F; lattice_columns::TOTAL]).add(row_idx) };
            write_blake3_state(row, lattice_columns::BLAKE3_ROUND, &state);

            let cv_out: [u32; 8] = if row_idx == pivot {
                std::array::from_fn(|i| state[i] ^ state[8 + i])
            } else {
                let init = state;
                let msg: [u32; 16] = read_from_trace(&trace[row_idx], lattice_columns::BLAKE3_MSG);
                let states = compute_blake3_round(&mut state, &msg);
                for (i, s) in states[..3].iter().enumerate() {
                    write_blake3_state(row, lattice_columns::BLAKE3_ROUND + (i + 1) * BLAKE3_STATE_SIZE, s);
                }
                std::array::from_fn(|i| {
                    if i < 4 {
            ...
            write_to_trace(row, lattice_columns::CV_OUT, &cv_out);
        }
    });
...
fn load_buffer<F: RichField>(
    ...
    match *data_source {
        ...
        MessageDataType::PreviousCv { source_row_idx } => {
            buffer[8..16].copy_from_slice(&read_from_trace::<F, u32, 8>(&trace[source_row_idx], lattice_columns::CV_OUT));
        }
        MessageDataType::Jackpot => {
            // Read JACKPOT_MSG from trace (filled by fill_cumsum_buffer_xor_jackpot)
            let jackpot_msg: [u32; 16] = read_from_trace(&trace[row_idx], lattice_columns::JACKPOT_MSG);
...
fn postprocess_round8_rows<F: RichField>(trace: &mut [[F; lattice_columns::TOTAL]], circuit: &[BlakeRoundLogic]) {
    let n = trace.len();
    for row_idx in 0..n {
        ...
        let base = lattice_columns::BLAKE3_ROUND;
        let msg: [u32; 16] = read_from_trace(&trace[row_idx], lattice_columns::BLAKE3_MSG);
