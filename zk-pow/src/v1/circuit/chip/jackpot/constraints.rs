...
use crate::v1::circuit::{
    ...
    lattice_layout::{lattice_columns, lattice_public},
    lattice_program::{JACKPOT_SIZE, LROT_PER_TILE},
    utils::{
    ...
    E: Evaluator<V, S>,
{
    ...
    debug_assert_eq!(row_view.offset, lattice_columns::CUMSUM_BUFFER);
    let cumsum_buffer = row_view.consume_few(lattice_columns::CUMSUM_BUFFER_LEN);
    let next_cumsum_buffer = &next_trace[lattice_columns::CUMSUM_BUFFER_RANGE];

    for i in 0..lattice_columns::CUMSUM_BUFFER_LEN {
        let expected = eval.mux(
            cf.is_dump_cumsum_buffer,
            next_cumsum_buffer[(i + 1) % lattice_columns::CUMSUM_BUFFER_LEN],
            cumsum_tile[i],
        );
        eval.constraint_eq(cumsum_buffer[i], expected);
    }
    debug_assert_eq!(row_view.offset, lattice_columns::CUMSUM_BUFFER_END);
    ...
    debug_assert_eq!(row_view.offset, lattice_columns::JACKPOT_MSG);
    let jackpot_msg = row_view.consume_few(lattice_columns::JACKPOT_MSG_LEN);
    let next_jackpot = &next_trace[lattice_columns::JACKPOT_MSG_RANGE];
    let next_jackpot_idx = &next_trace[lattice_columns::JACKPOT_IDX_RANGE];
    let next_bit_reg = &next_trace[lattice_columns::BIT_REG_RANGE];

    // jackpot initialized with 0's
    for i in 0..lattice_columns::JACKPOT_MSG_LEN {
    ...
    let next_is_load = next_trace[lattice_columns::IS_LOAD];
    ...
    let next_is_store_rot0 = next_trace[lattice_columns::IS_STORE0];
    let next_is_store_rot1 = next_trace[lattice_columns::IS_STORE1];
    let next_is_store_rot2 = next_trace[lattice_columns::IS_STORE2];
    ...
    debug_assert_eq!(row_view.offset, lattice_columns::BIT_REG);
    let bit_reg = row_view.consume_few(lattice_columns::BIT_REG_LEN);
    for &bit in bit_reg {
        eval.constraint_bool(bit);
    }

    // IS_XOR constraint: bit_reg XOR next_bit_reg must equal cumsum_buffer[0] (as i32)
    let next_is_xor = next_trace[lattice_columns::IS_XOR];
    ...
    let next_is_shift3 = next_trace[lattice_columns::IS_SHIFT3];
    ...
    debug_assert_eq!(row_view.offset, lattice_columns::BIT_REG_END);
}
