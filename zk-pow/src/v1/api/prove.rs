...
use crate::v1::circuit::lattice_circuit::{LatticeCircuitParams, LatticeRecursion, RecursionCircuit};
use crate::v1::circuit::lattice_layout::lattice_public;
use crate::v1::circuit::lattice_stark::LatticeStark;
...
pub fn prove_block(
    ...
    public_params.hash_jackpot = u32_field_array_to_hash(&stark_pis[lattice_public::HASH_JACKPOT_RANGE].try_into().unwrap());
    ...
    LatticeRecursion::compile_circuits(circuit_params, cache, true)?;

    let hash_public_data = public_params.public_data_commitment(&circuit_params);

    let proof = LatticeRecursion::prove(circuit_params, cache, (trace_rows, stark_pis, hash_public_data))?;
    Ok(proof)
}
