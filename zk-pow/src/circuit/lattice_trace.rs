...
use crate::circuit::lattice_layout::{BYTES_PER_GOLDILOCKS, lattice_columns, lattice_public};
use crate::circuit::lattice_noise::MMSlice;
use crate::circuit::lattice_preprocess::{generate_preprocessed, read_dword_from_matrix};
use crate::circuit::lattice_program::{TILE_D, TILE_H};
use crate::circuit::lattice_stark::{LatticeStarkChips, LatticeStarkConfig};
...
pub fn generate_trace<F: RichField + Extendable<D>, const D: usize>(
    chips: &LatticeStarkChips,
    config: &LatticeStarkConfig,
    ...
    let mut trace: Vec<[F; lattice_columns::TOTAL]> = (0..num_rows)
        .into_par_iter()
        .map(|_| [F::ZERO; lattice_columns::TOTAL])
        .collect();
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Fill PUBLIC_INPUTS
    let mut public_inputs = [F::ZERO; lattice_public::TOTAL];
    let mut public_inputs_builder = RowBuilder {
        row: &mut public_inputs,
        offset: lattice_public::JOB_KEY,
    };
    ...
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::JOB_KEY_END);
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Fill COMMITMENT_HASH (PUBLIC)
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::COMMITMENT_HASH);
    let (_, commitment_hash) = compiled_params.commitment_hash;
    for chunk in commitment_hash.chunks_exact(BYTES_PER_GOLDILOCKS) {
        public_inputs_builder.dump_u64(u64_pack_le(chunk, 8));
    }
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::COMMITMENT_HASH_END);

    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Fill HASH_A (PUBLIC)
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::HASH_A);
    for chunk in public_params.hash_a.chunks_exact(BYTES_PER_GOLDILOCKS) {
        public_inputs_builder.dump_u64(u64_pack_le(chunk, 8));
    }
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::HASH_A_END);

    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Fill HASH_B (PUBLIC)
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::HASH_B);
    for chunk in public_params.hash_b.chunks_exact(BYTES_PER_GOLDILOCKS) {
        public_inputs_builder.dump_u64(u64_pack_le(chunk, 8));
    }
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::HASH_B_END);

    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Fill HASH_ROUTING (PUBLIC). Zero-filled for non-MoE proofs.
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::HASH_ROUTING);
    ...
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::HASH_ROUTING_END);
    ...
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::HASH_JACKPOT);
    // Note: we could have just computed it as below: blake3(jackpot, commitment_hash)
    let jackpot_hash: [u32; 8] = read_from_trace(&trace[jackpot_row], lattice_columns::CV_OUT);
    for &word in &jackpot_hash {
        public_inputs_builder.dump_u64(word as u64);
    }
    debug_assert_eq!(public_inputs_builder.offset, lattice_public::HASH_JACKPOT_END);
    ...
    #[cfg(debug_assertions)]
    {
        ...
        let jackpot_msg_from_trace: [u32; 16] = read_from_trace(&trace[jackpot_row], lattice_columns::JACKPOT_MSG);
        assert_eq!(
        ...
        let expected_hash = lattice_blake3::blake3_digest(&jackpot_as_bytes, Some(commitment_hash));
...
fn blind_trace<F: RichField + Extendable<D>, const D: usize>(trace: &mut [[F; lattice_columns::TOTAL]], num_rows: usize) {
    let blinding_rows = [num_rows - 8, num_rows - 7, num_rows - 6];
    let random = F::rand_vec(blinding_rows.len() * lattice_columns::UINT8_DATA_LEN);
    for (&row_idx, chunk) in blinding_rows.iter().zip(random.chunks(lattice_columns::UINT8_DATA_LEN)) {
        for (c, f) in chunk.iter().enumerate() {
            trace[row_idx][lattice_columns::UINT8_DATA + c] = F::from_canonical_u64(f.to_canonical_u64() as u8 as u64);
        }
    }
}
...
fn fill_lookup_table_frequencies<F: RichField + Extendable<D>, const D: usize>(
    lookup_data: &[Lookup<F>],
    trace: &mut Vec<[F; lattice_columns::TOTAL]>,
    num_rows: usize,
) {
    let mut participating_columns = vec![false; lattice_columns::TOTAL];
