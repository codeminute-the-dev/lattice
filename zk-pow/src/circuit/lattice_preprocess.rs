...
use crate::circuit::lattice_layout::BITS_PER_OUTER_INDEX;
use crate::{
    api::proof_utils::{CompiledMoE, CompiledPublicParams},
    circuit::{
        lattice_layout::{BITS_PER_LIMB, NOISE_PACKING_BASE, lattice_columns},
        lattice_noise::{MMSlice, compute_noise},
        lattice_program::RowLogic,
...
pub(crate) fn generate_preprocessed(
    ...
    const CONTROL_BITS_LEN: usize = STATIC_BITS_LEN + lattice_columns::JACKPOT_IDX_LEN;
        ...
        .for_each(
            |(row_idx, ((((control_out, noise_out), cv_or_tweak_out), ab_id_out), outer_index))| {
                ...
                *outer_index = packed_outer;
                ...
                let muxer_bits = deg2_muxer_bits::<{ lattice_columns::JACKPOT_IDX_LEN }>(Some(if is_bitreg_load {
    ...
    // about ordering (e.g. `LatticeStark::preprocessed_columns`) must sort / look
    // up by the column index.
    let res = vec![
        (lattice_columns::CONTROL_PREP, control_prep),
        (lattice_columns::OUTER_INDICES_PACKED_PREP, outer_indices_packed_prep),
        (lattice_columns::NOISE_PACKED_PREP, noise_packed_prep),
        (lattice_columns::CV_OR_TWEAK_PREP, cv_or_tweak_prep),
        (lattice_columns::AB_ID_PREP, ab_id_prep),
    ];
