//! Lattice AIR: Constraint Definitions for the Lattice STARK
//! =====================================================
//!
//! The Lattice STARK trace has four interleaved chips sharing every row:
//!   Input, Blake3, Matmul, and Jackpot.
//!
//! Each row's behavior is determined by preprocessed control columns (see lattice_preprocess.rs),
//! which encode the program defined in lattice_program.rs. The Blake3 instruction stream and the
...
use crate::circuit::lattice_stark::LatticeStarkChips;
    ...
    E: Evaluator<V, S>,
{
    ...
    for i in 0..lattice_public::HASH_A_LEN {
        let pub_hash_a = eval.scalar(public_inputs[lattice_public::HASH_A + i]);
    ...
    for i in 0..lattice_public::HASH_B_LEN {
        let pub_hash_b = eval.scalar(public_inputs[lattice_public::HASH_B + i]);
        let out_cv = blake3_output[i];
        eval.constraint_eq_if(blake3_cf.is_hash_b, pub_hash_b, out_cv);
    }

    // Check HASH_JACKPOT agrees with blake3_output
    for i in 0..lattice_public::HASH_JACKPOT_LEN {
        let pub_hash_jackpot = eval.scalar(public_inputs[lattice_public::HASH_JACKPOT + i]);
        let out_cv = blake3_output[i];
        eval.constraint_eq_if(blake3_cf.is_hash_jackpot, pub_hash_jackpot, out_cv);
    }

    // Check HASH_ROUTING agrees with blake3_output
    for i in 0..lattice_public::HASH_ROUTING_LEN {
        let pub_hash_routing = eval.scalar(public_inputs[lattice_public::HASH_ROUTING + i]);
        let out_cv = blake3_output[i];
        eval.constraint_eq_if(blake3_cf.is_hash_routing, pub_hash_routing, out_cv);
    }

    debug_assert_eq!(row_view.offset, lattice_columns::TOTAL);
    row_view.assert_end();
}
