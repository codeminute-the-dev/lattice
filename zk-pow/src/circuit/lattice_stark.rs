...
use crate::circuit::lattice_air::eval_constraints;
use crate::circuit::lattice_layout::{lattice_columns, lattice_public};
use crate::circuit::lattice_preprocess::generate_preprocessed;
use crate::circuit::lattice_program::{RowLogic, TILE_D, TILE_H};
use crate::circuit::lattice_trace::generate_trace;
...
/// STARK for the Lattice proving system.
#[derive(Default, Clone, Debug)]
pub struct LatticeStark<F: RichField + Extendable<D>, const D: usize> {
    _phantom: std::marker::PhantomData<F>,
    pub chips: LatticeStarkChips,
    pub config: Option<LatticeStarkConfig>,
}

impl<F: RichField + Extendable<D>, const D: usize> Stark<F, D> for LatticeStark<F, D> {
    type EvaluationFrame<FE, P, const D2: usize>
        = StarkFrame<P, P::Scalar, { lattice_columns::TOTAL }, { lattice_public::TOTAL }>
    where
        FE: FieldExtension<D2, BaseField = F>,
        P: PackedField<Scalar = FE>;

    type EvaluationFrameTarget =
        StarkFrame<ExtensionTarget<D>, ExtensionTarget<D>, { lattice_columns::TOTAL }, { lattice_public::TOTAL }>;
    ...
    fn lookups(&self) -> Vec<Lookup<F>> {
        ...
        let mut lookup_data = vec![
            unfiltered_lookup(
                Column::<F>::single(lattice_columns::URANGE8_TABLE),
                Column::<F>::single(lattice_columns::URANGE8_FREQ),
                Column::<F>::singles(lattice_columns::UINT8_DATA_RANGE).collect(),
            ),
            unfiltered_lookup(
                Column::<F>::single(lattice_columns::URANGE13_TABLE),
                Column::<F>::single(lattice_columns::URANGE13_FREQ),
                Column::<F>::singles(
                    lattice_columns::MAT_ID_LIMBS_RANGE
                        .chain(lattice_columns::AB_ID_LIMBS_RANGE)
                        .chain(lattice_columns::OUTER_INDEX_FIRST_RANGE)
                        .chain(lattice_columns::OUTER_INDEX_SECOND_RANGE),
                )
                .collect(),
            ),
            // Signal is in [-64, 64].
            unfiltered_lookup(
                Column::<F>::single(lattice_columns::IRANGE7P1_TABLE),
                Column::<F>::single(lattice_columns::IRANGE7P1_FREQ),
                Column::<F>::singles(lattice_columns::MAT_UNPACK_RANGE.chain(lattice_columns::NOISE_UNPACK_RANGE)).collect(),
            ),
            unfiltered_lookup(
                Column::<F>::single(lattice_columns::IRANGE8_TABLE),
                Column::<F>::single(lattice_columns::IRANGE8_FREQ),
                Column::<F>::singles(lattice_columns::A_NOISED_UNPACK_RANGE.chain(lattice_columns::B_NOISED_UNPACK_RANGE))
                    .collect(),
            ),
            // Conversion: MAT_UNPACK: int7 -> UINT8_DATA: uint8, conditional on IS_MSG_MAT.
            Lookup {
                columns: lattice_columns::UINT8_DATA_RANGE
                    .zip(lattice_columns::MAT_UNPACK_RANGE)
                    .map(|(mu8, mi8)| indexed_column::<F>(mu8, mi8, 1 << 8, 0))
                    .collect(),
                table_column: Column::<F>::single(lattice_columns::I8U8_TABLE),
                frequencies_column: Column::<F>::single(lattice_columns::I8U8_FREQ),
                filter_columns: vec![
                    Filter::<F>::from_column(Column::<F>::single(lattice_columns::IS_MSG_BITS)); // IS_MSG_BIT_0 corresponds to IS_MSG_MAT
                    lattice_columns::UINT8_DATA_LEN
                ],
            },
        ];

        // RAM by indexing: load noised matrix tiles (A, B) indexed by (stark)row and (matrix)strip.
        let (num_words, num_rows, num_strips) = (lattice_columns::NOISED_PACKED_LEN, TILE_D / DWORD_SIZE, TILE_H);
        let noised_a_b = |word_idx, row_idx, strip_idx| {
            ...
            [
                indexed_column::<F>(
                    lattice_columns::A_NOISED + out_col,
                    lattice_columns::A_ID,
                    1u64 << 34,
                    idx_shift,
                ),
                indexed_column::<F>(
                    lattice_columns::B_NOISED + out_col,
                    lattice_columns::B_ID,
                    1u64 << 34,
                    idx_shift,
                ),
            ]
        ...
        for word_idx in 0..num_words {
            lookup_data.push(Lookup {
                columns: (0..num_rows)
                    ...
                    .collect(),
                table_column: indexed_column::<F>(
                    lattice_columns::NOISED_PACKED + word_idx,
                    lattice_columns::MAT_ID,
                    1u64 << 34,
                    0,
                ),
                frequencies_column: Column::<F>::single(lattice_columns::MAT_FREQ),
                filter_columns: vec![
                    Filter::<F>::from_column(Column::<F>::single(lattice_columns::IS_UPDATE_CUMSUM));
                    2 * num_rows * num_strips
                ],
            });
        }

        for i in 0..lattice_columns::CV_OUT_LEN {
            lookup_data.push(Lookup {
                columns: vec![indexed_column::<F>(
                    lattice_columns::CV_IN + i,
                    lattice_columns::CV_OR_TWEAK_PREP, // CV_OR_TWEAK_PREP serves as cv_idx
                    1u64 << 34,
                    0,
                )],
                frequencies_column: Column::<F>::single(lattice_columns::CV_OUT_FREQ),
                table_column: indexed_column::<F>(lattice_columns::CV_OUT + i, lattice_columns::STARK_ROW_IDX, 1u64 << 34, 0),
                filter_columns: vec![Filter::<F>::from_column(Column::<F>::single(lattice_columns::IS_CV_IN)); 1],
            });
        }
...
impl<F: RichField + Extendable<D>, const D: usize> LatticeStark<F, D> {
    pub fn new_with_params(public_params: &PublicProofParams) -> Self {
        Self {
            _phantom: PhantomData,
            chips: LatticeStarkChips::default(),
            config: Some(LatticeStarkConfig::new(public_params)),
        }
    }

    pub fn preprocessed_indices() -> SortedSet<usize> {
        SortedSet::from_unsorted(vec![
            lattice_columns::CONTROL_PREP,
            lattice_columns::OUTER_INDICES_PACKED_PREP,
            lattice_columns::NOISE_PACKED_PREP,
            lattice_columns::CV_OR_TWEAK_PREP,
            lattice_columns::AB_ID_PREP,
        ])
    }
    ...
    ) -> (Vec<[F; lattice_columns::TOTAL]>, [F; lattice_public::TOTAL]) {
