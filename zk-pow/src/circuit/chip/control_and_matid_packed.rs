...
use crate::circuit::{
    ...
    lattice_layout::{BITS_PER_LIMB, BITS_PER_OUTER_INDEX, lattice_columns},
    utils::{
        air_utils::RowView,
        evaluator::Evaluator,
...
impl ControlAndMatIDPackedChip {
        ...
        F: RichField + Extendable<D>,
    {
        ...
        row_builder.offset = lattice_columns::CONTROL_PREP;
        let control = row_builder.dump_noop().to_canonical_u64();
        // Unpack control bits (first 29 bits)
        let bits = u64_unpack_le(
            control,
            1,
            lattice_columns::JACKPOT_IDX_END - lattice_columns::IS_RESET_CUMSUM,
        );
        for &bit in bits.iter() {
            row_builder.dump_u64(bit);
        }
        debug_assert_eq!(row_builder.offset, lattice_columns::JACKPOT_IDX_END);

        // Fill MAT_ID_LIMBS
        let mat_id_from_control = control >> bits.len();
        let (mat_id_limb0, mat_id_limb1) = extract_and_dump(mat_id_from_control, row_builder);
        debug_assert_eq!(row_builder.offset, lattice_columns::MAT_ID_LIMBS_END);

        // Fill MAT_ID
        debug_assert_eq!(row_builder.offset, lattice_columns::MAT_ID);
        let mat_id = mat_id_limb0 + (mat_id_limb1 << BITS_PER_LIMB);
        row_builder.dump_u64(mat_id);
        debug_assert_eq!(row_builder.offset, lattice_columns::MAT_ID_END);
        ...
        debug_assert_eq!(row_builder.offset, lattice_columns::OUTER_INDEX_SECOND_END);
    }
        ...
        E: Evaluator<V, S>,
    {
        ...
        debug_assert_eq!(row_view.offset, lattice_columns::CONTROL_PREP);
        ...
        let mat_id_limbs = row_view.consume_few(lattice_columns::MAT_ID_LIMBS_LEN);
        ...
        debug_assert_eq!(row_view.offset, lattice_columns::MAT_ID_LIMBS_END);

        ///////////////////////////////////////////////////////////////////////////////////////////////////////////
        // Read MAT_ID (derived from CONTROL_PREP, used for tile lookups)
        debug_assert_eq!(row_view.offset, lattice_columns::MAT_ID);
        let mat_id = row_view.consume_single(); // Could have used MAT_ID_LIMBS directly.
        eval.constraint_eq(mat_id, expected_mat_id);
        debug_assert_eq!(row_view.offset, lattice_columns::MAT_ID_END);

        debug_assert_eq!(row_view.offset, lattice_columns::OUTER_INDICES_PACKED_PREP);
        ...
        debug_assert_eq!(row_view.offset, lattice_columns::OUTER_INDEX_SECOND_END);
