...
use crate::circuit::{
    ...
    lattice_layout::lattice_columns,
    lattice_program::LROT_PER_TILE,
...
pub fn fill_cumsum_buffer_xor_jackpot<F: RichField>(trace: &mut [[F; lattice_columns::TOTAL]], circuit: &[JackpotLogic]) {
    ...
    for &row_idx in &dump_rows {
        let cumsum: [i32; 4] = read_from_trace(&trace[row_idx], lattice_columns::CUMSUM_TILE);
        write_to_trace(&mut trace[row_idx], lattice_columns::CUMSUM_BUFFER, &cumsum);
    }
    ...
    for i in 0..dump_rows.len() {
        ...
        let mut current: [i32; 4] = read_from_trace(&trace[end_row], lattice_columns::CUMSUM_BUFFER);
        let mut row_idx = (end_row + num_rows - 1) % num_rows;
        while row_idx != start_row {
            current = [current[1], current[2], current[3], current[0]];
            write_to_trace(&mut trace[row_idx], lattice_columns::CUMSUM_BUFFER, &current);
    ...
    for row_idx in 0..num_rows {
        ...
        let prev_cumsum_buffer: [i32; 4] = if row_idx > 0 {
            read_from_trace(&trace[row_idx - 1], lattice_columns::CUMSUM_BUFFER)
        } else {
            [0i32; 4]
        };
        ...
        // Write to trace
        write_to_trace(&mut trace[row_idx], lattice_columns::JACKPOT_MSG, &jackpot_msg);
        write_u32_as_bits(&mut trace[row_idx], lattice_columns::BIT_REG, bit_reg);
    }
}
