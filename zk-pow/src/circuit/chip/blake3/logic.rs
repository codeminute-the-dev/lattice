...
///   lookup (filtered by bit0 directly in `lattice_stark.rs`). For Aux/Routing rows, UINT8_DATA
...
/// in `lattice_stark.rs` (int7→uint8 table), where the raw bit column is referenced directly.
