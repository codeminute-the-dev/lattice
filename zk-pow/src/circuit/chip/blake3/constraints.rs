...
use crate::circuit::{
    ...
    lattice_layout::{BYTES_PER_GOLDILOCKS, lattice_columns, lattice_public},
    utils::{air_utils::RowView, evaluator::Evaluator},
};

pub(crate) fn eval_constraints<V, S, E>(
    vars: &StarkFrame<V, S, { lattice_columns::TOTAL }, { lattice_public::TOTAL }>,
    eval: &mut E,
    ...
    E: Evaluator<V, S>,
{
    ...
    debug_assert_eq!(row_view.offset, lattice_columns::BLAKE3_MSG_BUFFER);
    let blake3_msg_buffer = row_view.consume_few(lattice_columns::BLAKE3_MSG_BUFFER_LEN);
    debug_assert_eq!(row_view.offset, lattice_columns::BLAKE3_MSG_BUFFER_END);
    ...
    debug_assert_eq!(row_view.offset, lattice_columns::CV_OR_TWEAK_PREP);
    let _cv_idx = row_view.consume_single(); // CV_OR_TWEAK_PREP serves as cv_idx through lookups
    debug_assert_eq!(row_view.offset, lattice_columns::CV_OR_TWEAK_PREP_END);

    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Read CV_IN
    debug_assert_eq!(row_view.offset, lattice_columns::CV_IN);
    let cv_in = row_view.consume_few(lattice_columns::CV_IN_LEN);
    debug_assert_eq!(row_view.offset, lattice_columns::CV_IN_END);

    // Check BLAKE3_MSG_BUFFER correctness (shift + data loading)
    let jackpot: [V; lattice_columns::JACKPOT_MSG_LEN] = local_trace[lattice_columns::JACKPOT_MSG_RANGE].try_into().unwrap();

    let (is_msg_jackpot, is_msg_uint8_data, is_msg_cv) = decode_is_msg_bits(eval, one, cf.is_msg_bits);
    verify_buffer_advancement(
        blake3_msg_buffer,
        &next_trace[lattice_columns::BLAKE3_MSG_BUFFER_RANGE],
        uint8_data,
        cv_in,
        &jackpot,
        is_msg_uint8_data,
    ...
    let blake3_tweak = next_trace[lattice_columns::CV_OR_TWEAK_PREP];

    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Read BLAKE3_MSG
    debug_assert_eq!(row_view.offset, lattice_columns::BLAKE3_MSG);
    let blake3_msg = row_view.consume_few(lattice_columns::BLAKE3_MSG_LEN);
    verify_msg_constraints(blake3_msg, blake3_msg_buffer, next_trace, cf.is_last_round, eval);
    debug_assert_eq!(row_view.offset, lattice_columns::BLAKE3_MSG_END);

    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Read BLAKE3_CV
    debug_assert_eq!(row_view.offset, lattice_columns::BLAKE3_CV);
    let blake3_cv = row_view.consume_few(lattice_columns::BLAKE3_CV_LEN);
    ...
    let job_key: [V; lattice_public::JOB_KEY_LEN] =
        std::array::from_fn(|i| eval.scalar(public_inputs[lattice_public::JOB_KEY + i]));
    let commitment_hash: [V; lattice_public::COMMITMENT_HASH_LEN] =
        std::array::from_fn(|i| eval.scalar(public_inputs[lattice_public::COMMITMENT_HASH + i]));
    ...
    for i in 0..lattice_columns::BLAKE3_CV_LEN {
    ...
    debug_assert_eq!(row_view.offset, lattice_columns::BLAKE3_CV_END);

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Read BLAKE3_ROUND and check constraints
    debug_assert_eq!(row_view.offset, lattice_columns::BLAKE3_ROUND);
    let next_is_new_blake = next_trace[lattice_columns::IS_NEW_BLAKE];
    let (blake3_output, init_state) = blake3_air::blake3_eval_transition_constraints(
        eval,
        row_view,
        blake3_msg,
        &next_trace[lattice_columns::BLAKE3_ROUND_RANGE],
        next_is_new_blake,
    );
    ...
    debug_assert_eq!(row_view.offset, lattice_columns::BLAKE3_ROUND_END);

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Read CV_OUT
    debug_assert_eq!(row_view.offset, lattice_columns::CV_OUT);
    let cv_out = row_view.consume_few(lattice_columns::CV_OUT_LEN);
    for i in 0..lattice_columns::CV_OUT_LEN {
        eval.constraint_eq(cv_out[i], blake3_output[i]);
    }
    debug_assert_eq!(row_view.offset, lattice_columns::CV_OUT_END);

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Read CV_OUT_FREQ
    debug_assert_eq!(row_view.offset, lattice_columns::CV_OUT_FREQ);
    let _cv_out_freq = row_view.consume_single();
    debug_assert_eq!(row_view.offset, lattice_columns::CV_OUT_FREQ_END);
...
where
    ...
    E: Evaluator<V, S>,
{
    let next_blake3_msg = &next_trace[lattice_columns::BLAKE3_MSG_RANGE];
    let next_is_new_blake = next_trace[lattice_columns::IS_NEW_BLAKE];
...
fn verify_buffer_advancement<V, S, E>(
    ...
    E: Evaluator<V, S>,
{
    let buffer_len = lattice_columns::BLAKE3_MSG_BUFFER_LEN; // 16
    ...
    for i in 0..lattice_columns::CV_IN_LEN {
        eval.constraint_eq_if(is_msg_cv, msg_buffer[8 + i], cv_in[i]);
    }

    debug_assert_eq!(jackpot.len(), lattice_columns::BLAKE3_MSG_LEN);
    for i in 0..lattice_columns::BLAKE3_MSG_LEN {
