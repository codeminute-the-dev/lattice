...
use lattice_blake3::{B3F_CHUNK_END, B3F_CHUNK_START, B3F_KEYED_HASH, B3F_PARENT, B3F_ROOT, BLAKE3_CHUNK_LEN, BLAKE3_MSG_LEN};
...
use crate::{
    ...
    circuit::{
        ...
        lattice_preprocess::read_blake_msg_from_matrix,
    },
    ensure_eq,
};
...
impl BlakeProgram {
    pub fn new(params: &PublicProofParams) -> (Self, Vec<AuxiliaryMsgLocation>, Vec<AuxiliaryCvLocation>) {
        ...
        let cv = recursive_compilation(
            0,
            lattice_blake3::padded_chunk_len(m * common_dim),
            m,
            common_dim,
        ...
        let cv = recursive_compilation(
            0,
            lattice_blake3::padded_chunk_len(total_b_cols * common_dim),
            total_b_cols,
            common_dim,
        ...
        if let Some(moe) = params.moe.as_ref() {
            ...
            let cv = recursive_compilation(
                0,
                lattice_blake3::padded_chunk_len(num_rows * routing_row_len),
                num_rows,
                routing_row_len,
                &hotspots,
                routing_row_len,
...
fn recursive_compilation(
    ...
    let is_root = start == 0 && end == lattice_blake3::padded_chunk_len(num_rows * row_len);
