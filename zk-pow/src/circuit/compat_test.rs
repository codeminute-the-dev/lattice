...
mod test {
    ...
    use crate::circuit::lattice_circuit::{LatticeRecursion, RecursionCircuit};
    use crate::circuit::lattice_stark::LatticeStark;
    ...
    fn starky_hash(block_header: IncompleteBlockHeader, plain_proof: &PlainProof) -> String {
        ...
        let stark = LatticeStark::<GoldilocksField, 2>::new_with_params(&public_params);
    ...
    /// Symbolically evaluate the Lattice AIR constraints, returning all constraint
    /// polynomials as human-readable string expressions.
    fn air_constraints_string(lattice: &LatticeStark<GoldilocksField, 2>) -> String {
        use crate::circuit::lattice_air::eval_constraints;
        use crate::circuit::lattice_layout::{lattice_columns, lattice_public};
        use starky::evaluation_frame::{StarkEvaluationFrame, StarkFrame};

        let num_cols = lattice_columns::TOTAL;
        let num_pis = lattice_public::TOTAL;

        let (mut evaluator, local_ids, next_ids, pi_ids) = StringEvaluator::new(num_cols, num_cols, num_pis);

        type Frame = StarkFrame<usize, usize, { lattice_columns::TOTAL }, { lattice_public::TOTAL }>;
        let frame = Frame::from_values(&local_ids, &next_ids, &pi_ids);

        eval_constraints::<_, _, StringEvaluator>(&lattice.chips, &frame, &mut evaluator);
    ...
    fn generate_and_write_stark_proof(proof_path: &Path) {
        let mut cache = <LatticeRecursion as RecursionCircuit>::CircuitCache::default();
