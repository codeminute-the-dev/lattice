// This file describe the high-level recursive structure of the Lattice proof.
...
//
//    Each row of the STARK trace has the following three interleaved chips (see lattice_program.rs):
...
//     Constraints:
//          a. Stark's proof is correct, with the specific lattice STARK's constraints.
...
use crate::{
    ...
    circuit::{
        ...
        lattice_layout::{BITS_PER_LIMB, lattice_columns, lattice_public},
        lattice_stark::LatticeStark,
    },
    ensure_eq,
};
use anyhow::ensure;
...
/// API for Lattice Recursion Circuit. Not thread-safe.
...
/// Lattice Recursion implementation for three-layered circuit architecture
#[derive(Clone, Copy, Debug, Default)]
pub struct LatticeRecursion;

// Type definitions for LatticeRecursion
impl RecursionCircuit for LatticeRecursion {
    const EXT_D: usize = 2;
    ...
    type StarkTrace = (
        Vec<[Self::F; lattice_columns::TOTAL]>,
        [Self::F; lattice_public::TOTAL],
        HashOut<Self::F>,
    );
    type CircuitParams = LatticeCircuitParams;
    type VerifierPIs = LatticeVerifierPIs;
    ...
    fn stark_config(params: Self::CircuitParams) -> StarkConfig {
        StarkConfig {
            ...
            preprocessed_columns: LatticeStark::<Self::F, { Self::EXT_D }>::preprocessed_indices(),
        }
    }
    ...
    fn compile_circuits(params: Self::CircuitParams, cache: &mut Self::CircuitCache, store_prover_data: bool) -> Result<()> {
        ...
        let stark = LatticeStark::<Self::F, { Self::EXT_D }>::default();
        ...
        if !first_in_cache {
            ...
            ensure_eq!(proof_0_target.public_inputs.len(), lattice_public::TOTAL);

            // Layer-1 PI layout: lattice_public | hash_public_data (4) | zeta (2) |
            //                    evals_at_zeta (2*N) | evals_at_g_zeta (2*N)
            // lattice_public (JOB_KEY, COMMITMENT_HASH, HASH_A, HASH_B, HASH_ROUTING, HASH_JACKPOT) of stark must PIs given outside
            ...
            let stark_zeta =
                verify_stark_proof_circuit::<Self::F, Self::InnerC, LatticeStark<Self::F, { Self::EXT_D }>, { Self::EXT_D }>(
                    &mut builder_1,
                    stark,
                    &proof_0_target,
                    &stark_config,
