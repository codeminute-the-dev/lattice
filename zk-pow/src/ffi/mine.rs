...
use crate::circuit::lattice_noise::compute_noise_for_indices;
use crate::circuit::lattice_program::{JACKPOT_SIZE, LROT_PER_TILE};
...
pub fn try_mine_one<R: Rng>(
    ...
    let a_row_major = lattice_blake3::pad_to_chunk_boundary(&flatten_matrix(&a_matrix));
    let b_col_major = lattice_blake3::pad_to_chunk_boundary(&flatten_matrix(&b_transposed));
    ...
    // Compute noise using shared implementation from lattice_noise
...
/// Build a MatrixMerkleProof for the given matrix and row indices using lattice_blake3.
fn build_matrix_proof(matrix: &[Vec<i8>], job_key: &[u8; 32], row_indices: &[usize], num_cols: usize) -> MatrixMerkleProof {
    let padded = lattice_blake3::pad_to_chunk_boundary(&flatten_matrix(matrix));
    let tree = lattice_blake3::MerkleTree::new(&padded, *job_key);
    let leaf_indices = lattice_blake3::MerkleTree::compute_leaf_indices_from_rows(row_indices, (matrix.len(), num_cols));
...
fn routing_hash(routing: &[Vec<u32>], job_key: &[u8; 32]) -> [u8; 32] {
    let padded = lattice_blake3::pad_to_chunk_boundary(&flatten_routing(routing));
...
fn build_routing_proof(
    ...
    let leaf_indices =
        lattice_blake3::MerkleTree::compute_leaf_indices_from_rows(&row_indices, (total_entries, std::mem::size_of::<u32>()));
    let proof = tree.get_multileaf_proof(&leaf_indices);
...
mod tests {
    ...
    fn test_build_matrix_proof_pads_to_chunk_boundary() {
        ...
        let padded = lattice_blake3::pad_to_chunk_boundary(&flatten_matrix(&matrix));
        let expected_root = lattice_blake3::Blake3Hasher::with_key(key).hash(&padded);
        assert_eq!(
    ...
    fn test_build_matrix_proof_aligned_unchanged() {
        ...
        let expected_root = lattice_blake3::Blake3Hasher::with_key(key).hash(&flat);
        assert_eq!(
            proof.proof.root, expected_root,
    ...
    fn test_padded_blake3_hash_equals_merkle_root() {
        ...
        let hash_via_tree = lattice_blake3::MerkleTree::new(&padded, key).root();
    ...
    fn test_moe_commitment_matches_verifier_non_aligned_routing() {
        ...
        let a_row_major = lattice_blake3::pad_to_chunk_boundary(&[1u8; 100]);
        let b_col_major = lattice_blake3::pad_to_chunk_boundary(&[2u8; 100]);
