...
use lattice_blake3::MerkleProof;
...
use lattice_blake3::{BLAKE3_CHUNK_LEN, BLAKE3_DIGEST_SIZE};
...
fn compute_external_cvs(
    ...
    let a_ranges = p.a.proof.compute_sibling_ranges(lattice_blake3::padded_chunk_len(p.m * k));
    let b_ranges =
        p.bt.proof
            .compute_sibling_ranges(lattice_blake3::padded_chunk_len(total_b_cols * k));
    ...
    let routing = p.moe.as_ref().map(|moe| {
        ...
        let ranges = moe
            .routing_proof
            .compute_sibling_ranges(lattice_blake3::padded_chunk_len(raw_len));
...
impl PlainProof {
    ...
    fn expected_merkle_leaves(dims: &[usize]) -> Result<usize> {
        ...
        Ok(lattice_blake3::padded_chunk_len(bytes) / BLAKE3_CHUNK_LEN)
    }
