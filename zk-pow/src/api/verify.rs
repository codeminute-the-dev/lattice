...
use crate::{
    ...
    circuit::{
        ...
        lattice_circuit::{LatticeCircuitParams, LatticeRecursion, LatticeVerifierPIs, RecursionCircuit},
        lattice_noise::compute_noise,
...
pub fn verify_block(public_params: &PublicProofParams, proof: &ZKProof, cache: &mut CircuitCache) -> Result<()> {
    ...
    LatticeRecursion::compile_circuits(params, cache, false)?;
...
fn verify_with_cache(
    params: LatticeCircuitParams,
    cache: &CircuitCache,
    pis: &LatticeVerifierPIs,
    proof: &ZKProof,
) -> Result<()> {
    if LatticeRecursion::verify(params, cache, pis.clone(), &proof.plonky2_proof)? {
        Ok(())
    } else {
        bail!("Proof Invalid")
    }
...
fn prepare_verification(
    ...
    proof: &ZKProof,
    nbits_override: Option<u32>,
) -> Result<(LatticeCircuitParams, LatticeVerifierPIs)> {
    ...
    let circuit_params = LatticeCircuitParams {
...
fn build_verifier_pis(
    ...
    circuit_params: &LatticeCircuitParams,
    proof: &ZKProof,
) -> Result<LatticeVerifierPIs> {
    ...
    Ok(LatticeVerifierPIs {
        ...
        zeta: proof.zeta()?,
        preprocessed_columns: LatticeStark::<GoldilocksField, 2>::preprocessed_columns(compiled_params)?,
    })
}
