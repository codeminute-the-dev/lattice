...
const (
	...
	// generated via latticed.
	CoinbaseFlags = "/P2SH/latticed/"
)
...
type TxDesc struct {
	...
	// FeePerKB is the fee the transaction pays in Cell per 1000 bytes.
	FeePerKB int64
}
...
// (cells per vbyte) and is already initialized for use with heap.Push/Pop.
...
//   - Fee rate (cells per kilobyte) - HIGHEST FIRST
//
...
mempoolLoop:
	...
	case wire.CertificateVersionV4:
		certificate = &wire.CertificateV4{CertificateV3: wire.CertificateV3{
			CertificateV2: wire.CertificateV2{Hash: msgBlock.BlockHash()},
		}}
