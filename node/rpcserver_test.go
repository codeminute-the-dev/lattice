...
func TestHandleTestMempoolAcceptMixedResults(t *testing.T) {
	...
	expectedResults[2] = &btcjson.TestMempoolAcceptResult{
		...
		Fees: &btcjson.TestMempoolAcceptFees{
			Base:             feeSats.ToLATT(),
			EffectiveFeeRate: feeSats.ToLATT() * 1e3 / 100,
		},
	}

	// Create a mock request with default max fee rate of 0.1 LATT/KvB.
...
func TestHandleTestMempoolAcceptFees(t *testing.T) {
	...
	const (
		// Set transaction's fee rate to be 0.2LATT/kvB.
	...
	testCases := []struct {
		...
		},
		{
			...
			// 0.1LATT/kvB is used and the fee rate(0.2) used by the
