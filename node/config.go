...
import (
	...
	_ "github.com/codeminute-the-dev/lattice/node/database/ffldb"
...
const (
	defaultConfigFilename           = "latticed.conf"
	defaultDataDirname              = "data"
	defaultLogLevel                 = "info"
	defaultLogDirname               = "logs"
	defaultLogFilename              = "latticed.log"
	...
	sampleConfigFilename            = "sample-latticed.conf"
...
var (
	defaultHomeDir     = btcutil.AppDataDir("latticed", false)
...
// config defines the configuration options for latticed.
//
...
type config struct {
	...
	MinRelayTxFee            float64       `long:"minrelaytxfee" description:"The minimum transaction fee in LATT/kB to be considered a non-zero fee."`
...
//
// The above results in latticed functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	cfg := config{
		...
		MinRelayTxFee:            mempool.DefaultMinRelayTxFee.ToLATT(),
	...
	if cfg.DisableRPC {
		latdLog.Infof("RPC service is disabled")
	}
	...
	if configFileError != nil {
		latdLog.Warnf("%v", configFileError)
	}
...
// latticedDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func latticedDial(addr net.Addr) (net.Conn, error) {
...
// latticedLookup resolves the IP of the given host using the correct DNS lookup
...
func latticedLookup(host string) ([]net.IP, error) {
