...
const (
	...
	// defaultMaxFeeRate is the default value to use(0.1 LATT/kvB) when the
...
var rpcHandlersBeforeInit = map[string]commandHandler{
	...
	"getnextreset":           handleGetNextReset,
...
// list of commands that we recognize, but for which latticed has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of Latwallet.
...
// These commands will be implemented in Latwallet.
...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.LatEncode(&buf, maxProtocolVersion, wire.WitnessEncoding); err != nil {
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount*btcutil.CellPerLatt > btcutil.MaxCell {
			return nil, &btcjson.RPCError{
		...
		// Convert the amount to cell.
		cell, err := btcutil.NewAmount(amount)
		if err != nil {
			context := "Failed to convert amount"
			return nil, internalRPCError(err.Error(), context)
		}

		txOut := wire.NewTxOut(int64(cell), pkScript)
		mtx.AddTxOut(txOut)
	}
...
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []btcjson.Vout {
	...
	for i, v := range mtx.TxOut {
		...
		vout.N = uint32(i)
		vout.Value = btcutil.Amount(v.Value).ToLATT()
...
// handleGetNextReset implements the getnextreset command.
//
// Everything reported here is derived from hardcoded consensus constants and
// the current height, so any node answers identically. The reset height is
// exact; only the time estimates assume blocks keep arriving at the target
// interval.
func handleGetNextReset(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	best := s.cfg.Chain.BestSnapshot()
	params := s.cfg.ChainParams
	e := &params.Emission
	height := best.Height

	toLATT := func(cells int64) float64 {
		return btcutil.Amount(cells).ToLATT()
	}

	blocksLeft := blockchain.BlocksUntilReset(height, e)
	secsLeft := int64(blocksLeft) * int64(params.TargetTimePerBlock/time.Second)
	inEpoch := blockchain.HeightInEpoch(height, e)

	return &btcjson.GetNextResetResult{
		Height:           height,
		Epoch:            blockchain.EpochIndex(height, e),
		EpochBlocks:      e.EpochBlocks,
		HeightInEpoch:    inEpoch,
		EpochStartHeight: blockchain.EpochStartHeight(blockchain.EpochIndex(height, e), e),

		NextResetHeight:            blockchain.NextResetHeight(height, e),
		BlocksUntilReset:           blocksLeft,
		EstimatedSecondsUntilReset: secsLeft,
		EstimatedResetTime:         time.Now().Unix() + secsLeft,

		BlocksSinceLastReset:           inEpoch,
		EstimatedSecondsSinceLastReset: inEpoch * int64(params.TargetTimePerBlock/time.Second),

		CurrentReward: toLATT(blockchain.CalcBlockSubsidy(height, params)),
		NextReward:    toLATT(blockchain.CalcBlockSubsidy(height+1, params)),
		InitialReward: toLATT(e.InitialSubsidy()),

		MintedThisEpoch: toLATT(blockchain.SupplyMintedInEpochThrough(height, e)),
		ResetThreshold:  toLATT(e.ResetThreshold),
		TotalSupply:     toLATT(blockchain.TotalSupplyThrough(height, params)),

		TargetBlockSeconds: params.TargetTimePerBlock.Seconds(),
	}, nil
}
...
func handleGetBlockTemplateRequest(s *rpcServer, request *btcjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &btcjson.RPCError{
			...
			Message: "Lattice is not connected",
		}
	}
	...
	if currentHeight != 0 && !s.cfg.Chain.IsCurrent() {
		return nil, &btcjson.RPCError{
			...
			Message: "Lattice is downloading blocks...",
		}
	}
...
//
// NOTE: This is a Lattice extension originally ported from
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &btcjson.InfoChainResult{
		Version:         int32(1000000*latticeversion.Major + 10000*latticeversion.Minor + 100*latticeversion.Patch),
		...
		TestNet:         cfg.TestNet,
		RelayFee:        cfg.minRelayTxFee.ToLATT(),
	}
...
func getLocalServiceNames(services wire.ServiceFlag) []string {
	// Map latticed service flags to their human-readable names.
...
func handleGetNetworkInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &btcjson.GetNetworkInfoResult{
		...
		Networks:           networks,
		RelayFee:           cfg.minRelayTxFee.ToLATT(),
		IncrementalFee:     cfg.minRelayTxFee.ToLATT(),
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	txOutReply := &btcjson.GetTxOutResult{
		...
		Value:         btcutil.Amount(value).ToLATT(),
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &btcjson.PrevOut{
				...
				Value:     btcutil.Amount(originTxOut.Value).ToLATT(),
			}
		}
	}
...
const messageSignatureHeader = "Lattice Signed Message:\n"
...
//
// NOTE: This is a Lattice extension ported from github.com/decred/dcrd.
...
func validateFeeRate(feeSats btcutil.Amount, txSize int64,
	...
	// Convert sats/vB to LATT/kvB.
	feeRate := feeRateSatsPerKVB.ToLATT()

	// Get the max fee rate, if not provided, default to 0.1 LATT/kvB.
	if maxFeeRate == 0 {
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err == nil {
			...
			//
			// Latticed does not respond to any request without and "id" or "id":null,
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="latticed RPC"`)
