...
import (
	...
	"github.com/codeminute-the-dev/lattice/node/addrmgr"
	...
	"github.com/codeminute-the-dev/lattice/node/mining/cpuminer"
	"github.com/codeminute-the-dev/lattice/node/netsync"
	...
	latticeversion "github.com/codeminute-the-dev/lattice/version"
...
var (
	...
	userAgentName = "latticed"

	// userAgentVersion is the user agent version and is used to help
	// identify ourselves to other peers on the network.
	userAgentVersion = latticeversion.UserAgent()
)
...
// server provides a Lattice server for handling communications to and from
// peers.
type server struct {
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > btcutil.MaxCell {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			latticedLookup, func(addrs []*wire.NetAddressV2) {
	...
	if !cfg.DisableTLS {
		...
		if !fileExists(cfg.RPCKey) && !fileExists(cfg.RPCCert) {
			...
			keyPair, err = btcutil.WriteTLSCertPair(cfg.RPCCert,
				cfg.RPCKey, "latticed autogenerated cert", validUntil,
				nil, true)
			if err != nil {
				return nil, err
			}
...
// newServer returns a new latticed server configured to listen on addr for the
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	amgr := addrmgr.New(cfg.DataDir, latticedLookup)
	...
	if cfg.Prune != 0 {
		latdLog.Infof("Prune set to %d MiB", cfg.Prune)
	}
