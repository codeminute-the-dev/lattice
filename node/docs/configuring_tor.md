...
latticed provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, latticed supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the Lattice
...
Configuring latticed as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the latticed command
line or in the latticed configuration file.  Typically the Tor proxy address will be
...
By default, latticed assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since latticed will act as the server.
...
```bash
./latticed --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
latticed provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the latticed command line or in the latticed
...
```bash
./latticed --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
latticed provides support for Tor stream isolation by using the `--torisolation`
flag.  This option requires --proxy or --onionproxy to be set.

### Command line example

```bash
./latticed --proxy=127.0.0.1:9050 --torisolation
```
