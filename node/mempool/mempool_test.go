	...
	harness := poolHarness{
		...
		txPool: New(&Config{
			Policy: Policy{
				...
				MinRelayTxFee:        1000, // 1 Cell per byte
				MaxTxVersion:         1,
			},
			ChainParams:      chainParams,
...
func TestRBF(t *testing.T) {
	...
	// A fee small enough to sit comfortably inside a Lattice coinbase.
	// Pearl paid thousands of coins per block, so a 1 PRL test fee was
	// negligible; Lattice pays 2 LATT, so the fee has to scale down with it
	// or a replacement transaction ends up with a negative output.
	const defaultFee = btcutil.CellPerLatt / 100
