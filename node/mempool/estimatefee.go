...
import (
	...
	"github.com/codeminute-the-dev/lattice/node/mining"
)
...
// CellPerByte is number with units of cells per byte.
type CellPerByte float64

// LatPerKilobyte is number with units of lattices per kilobyte.
type LatPerKilobyte float64

// ToLatPerKb returns a float value that represents the given
// CellPerByte converted to cells per kb.
func (rate CellPerByte) ToLatPerKb() LatPerKilobyte {
	// If our rate is the error value, return that.
	if rate == CellPerByte(-1.0) {
		return -1.0
	}

	return LatPerKilobyte(float64(rate) * bytePerKb * latPerCell)
}

// Fee returns the fee for a transaction of a given size for
// the given fee rate.
func (rate CellPerByte) Fee(size uint32) btcutil.Amount {
	// If our rate is the error value, return that.
	if rate == CellPerByte(-1) {
...
// NewCellPerByte creates a CellPerByte from an Amount and a
// size in bytes.
func NewCellPerByte(fee btcutil.Amount, size uint32) CellPerByte {
	return CellPerByte(float64(fee) / float64(size))
}
...
type observedTransaction struct {
	...
	// The fee per byte of the transaction in cells.
	feeRate CellPerByte
...
func (ef *FeeEstimator) ObserveTransaction(t *TxDesc) {
	...
	if _, ok := ef.observed[hash]; !ok {
		...
		ef.observed[hash] = &observedTransaction{
			hash:     hash,
			feeRate:  NewCellPerByte(btcutil.Amount(t.Fee), size),
			observed: t.Height,
...
func (b *estimateFeeSet) estimateFee(confirmations int) CellPerByte {
	if confirmations <= 0 {
		return CellPerByte(math.Inf(1))
	}
...
func (ef *FeeEstimator) newEstimateFeeSet() *estimateFeeSet {
	...
	set.feeRate = make([]CellPerByte, capacity)
...
func (ef *FeeEstimator) estimates() []CellPerByte {
	set := ef.newEstimateFeeSet()

	estimates := make([]CellPerByte, estimateFeeDepth)
...
func (ef *FeeEstimator) EstimateFee(numBlocks uint32) (LatPerKilobyte, error) {
	ef.mtx.Lock()
	defer ef.mtx.Unlock()
	...
	return ef.cached[int(numBlocks)-1].ToLatPerKb(), nil
}
