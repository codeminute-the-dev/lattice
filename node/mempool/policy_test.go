...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		{
			// Lattice's relay floor is deliberately near zero (10
			// cells/kB), so an ordinary transaction costs a rounding
			// error rather than a real fee.
		...
		{
			"max standard tx vsize with max cell relay fee",
			maxStandardTxVsize,
			btcutil.MaxCell,
			btcutil.MaxCell,
		},
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			"max cell amount is never dust",
			wire.TxOut{Value: btcutil.MaxCell, PkScript: pkScript},
			btcutil.MaxCell,
			false,
		},
		{
