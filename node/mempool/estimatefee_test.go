...
func expectedFeePerKilobyte(t *TxDesc) LatPerKilobyte {
	size := float64(t.Tx.MsgTx().SerializeSize())
	fee := float64(t.Fee)

	return CellPerByte(fee / size).ToLatPerKb()
}
...
func (eft *estimateFeeTester) estimates() [estimateFeeDepth]LatPerKilobyte {

	// Generate estimates
	var estimates [estimateFeeDepth]LatPerKilobyte
...
func (eft *estimateFeeTester) round(txHistory [][]*TxDesc,
	estimateHistory [][estimateFeeDepth]LatPerKilobyte,
	txPerRound, txPerBlock uint32) ([][]*TxDesc, [][estimateFeeDepth]LatPerKilobyte) {
...
func TestEstimateFeeRollback(t *testing.T) {
	...
	estimateHistory := [][estimateFeeDepth]LatPerKilobyte{eft.estimates()}
...
func (eft *estimateFeeTester) checkSaveAndRestore(
	previousEstimates [estimateFeeDepth]LatPerKilobyte) {
