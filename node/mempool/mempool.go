...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in LATT/kB to be
...
func (mp *TxPool) RawMempoolVerbose() map[string]*btcjson.GetRawMempoolVerboseResult {
	...
	for _, desc := range mp.pool {
		...
		mpd := &btcjson.GetRawMempoolVerboseResult{
			...
			Fee:     btcutil.Amount(desc.Fee).ToLATT(),
...
type MempoolAcceptResult struct {
	// TxFee is the fees paid in cell.
...
func (mp *TxPool) trimToSize() {
	...
	if nRemoved > 0 {
		log.Debugf("Evicted %d txns from mempool (pool size: %d bytes), "+
			"fee floor now %d cell/kB", nRemoved, mp.poolBytes,
			mp.lastEvictedFeeRate)
	}
}
