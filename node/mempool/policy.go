...
const (
	...
	// DefaultMinRelayTxFee is the minimum fee in cell that is required
	// for a transaction to be treated as free for relay and mining
	// purposes.  It is also used to help determine if a transaction is
	// considered dust and as a base for calculating minimum required fees
	// for larger transactions.  This value is in Cell/1000 bytes.
...
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee btcutil.Amount) int64 {
	...
	if minFee < 0 || minFee > btcutil.MaxCell {
		minFee = btcutil.MaxCell
	}
...
func IsDust(txOut *wire.TxOut, minRelayTxFee btcutil.Amount) bool {
	...
	// minFreeTxRelayFee is in Cell/KB, so multiply by 1000 to
	// convert to bytes.
	//
	// Using the typical values for a pay-to-pubkey-hash transaction from
	// the breakdown above and the default minimum free transaction relay
	// fee of 1000, this equates to values less than 546 cell being
	// considered dust.
	dustThreshold := GetDustThreshold(txOut)
	feeRate := int64(minRelayTxFee)

	if txOut.Value > btcutil.MaxCell {
		return true
	}
