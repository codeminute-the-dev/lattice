...
// LatticeNet is a type used to represent the Lattice network that we're
// connecting to.
//
// NOTE: This is identical to the wire.LatticeNet type, but allows us to shed a
...
func (p *Peer) createV2Ciphers(ecdhSecret []byte, initiating bool,
	net LatticeNet) error {

	log.Debugf("Creating v2 ciphers (initiating=%v, net=%v)", initiating,
		net)

	// Define the salt as the string "lattice_v2_shared_secret" followed by
	// the LatticeNet's "magic" bytes.
	salt := []byte("lattice_v2_shared_secret")
...
func (p *Peer) CompleteHandshake(initiating bool, decoyContentLens []int,
	...
	err = p.createV2Ciphers(ecdhSecret[:], initiating, latnet)
	if err != nil {
		return err
	}
