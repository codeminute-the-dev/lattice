...
// MsgGetData implements the Message interface and represents a
...
func (msg *MsgGetData) LatDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxInvPerMsg {
		...
		return messageError("MsgGetData.LatDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgGetData) LatEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	// Limit to max inventory vectors per message.
	count := len(msg.InvList)
	if count > MaxInvPerMsg {
		str := fmt.Sprintf("too many invvect in message [%v]", count)
		return messageError("MsgGetData.LatEncode", str)
	}
