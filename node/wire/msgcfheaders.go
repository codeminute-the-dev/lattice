...
func (msg *MsgCFHeaders) LatDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			...
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.LatDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgCFHeaders) LatEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	count := len(msg.FilterHashes)
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			"message [count %v, max %v]", count,
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.LatEncode", str)
	}
...
// differs from LatDecode in that LatDecode decodes from the wire
