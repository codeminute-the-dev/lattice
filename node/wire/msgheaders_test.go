...
func TestHeadersWire(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := test.in.LatEncode(&buf, test.pver, test.enc)
		require.NoError(err, "LatEncode #%d error", i)
		require.Equal(test.buf, buf.Bytes(), "LatEncode #%d", i)

		// Decode the message from wire format.
		var msg MsgHeaders
		rbuf := bytes.NewReader(test.buf)
		err = msg.LatDecode(rbuf, test.pver, test.enc)
		require.NoError(err, "LatDecode #%d error", i)
		require.Equal(test.out, &msg, "LatDecode #%d", i)
	}
}
...
func TestHeadersNullCertWire(t *testing.T) {
	...
	// Encode.
	var buf bytes.Buffer
	err := nullCertHeader.LatEncode(&buf, ProtocolVersion, BaseEncoding)
	require.NoError(err, "LatEncode null cert")
	require.Equal(nullCertHeaderEncoded, buf.Bytes(), "LatEncode null cert bytes")

	// Decode.
	var decoded MsgHeaders
	rbuf := bytes.NewReader(nullCertHeaderEncoded)
	err = decoded.LatDecode(rbuf, ProtocolVersion, BaseEncoding)
	require.NoError(err, "LatDecode null cert")
...
func TestHeadersWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := test.in.LatEncode(w, test.pver, test.enc)
		require.IsType(test.writeErr, err, "LatEncode #%d wrong error type", i)

		// For errors which are not of type MessageError, check them for
		// equality.
		if _, ok := err.(*MessageError); !ok {
			require.Equal(test.writeErr, err, "LatEncode #%d wrong error", i)
		}

		// Decode from wire format.
		var msg MsgHeaders
		r := newFixedReader(test.max, test.buf)
		err = msg.LatDecode(r, test.pver, test.enc)
		require.IsType(test.readErr, err, "LatDecode #%d wrong error type", i)

		// For errors which are not of type MessageError, check them for
		// equality.
		if _, ok := err.(*MessageError); !ok {
			require.Equal(test.readErr, err, "LatDecode #%d wrong error", i)
		}
	}
}
