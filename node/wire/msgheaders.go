...
func (mh *MsgHeader) LatDecode(r io.Reader, pver uint32, buf []byte) error {
	if err := mh.MsgCertificate.LatDecode(r, pver); err != nil {
		return err
	}
...
func (mh *MsgHeader) LatEncode(w io.Writer, pver uint32, buf []byte) error {
	if err := mh.MsgCertificate.LatEncode(w, pver); err != nil {
		return err
	}
...
func (msg *MsgHeaders) LatDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	buf := binarySerializer.Borrow()
	defer binarySerializer.Return(buf)
	...
	if count > MaxBlockHeadersPerMsg {
		...
		return messageError("MsgHeaders.LatDecode", str)
	}
	...
	if HasInconsistentCertificates(msg.Headers) {
		return messageError("MsgHeaders.LatDecode",
...
// This is part of the Message interface implementation.
func (msg *MsgHeaders) LatEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	// Limit to max block headers per message.
	count := len(msg.Headers)
	if count > MaxBlockHeadersPerMsg {
		str := fmt.Sprintf("too many block headers for message "+
			"[count %v, max %v]", count, MaxBlockHeadersPerMsg)
		return messageError("MsgHeaders.LatEncode", str)
	}
	...
	for i := range msg.Headers {
		err := msg.Headers[i].LatEncode(w, pver, buf)
		if err != nil {
			return err
		}
	}
