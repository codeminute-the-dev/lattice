...
// LatDecode decodes r using the wire protocol encoding into the receiver.
// This is part of the Message interface implementation.
// See Deserialize for decoding blocks stored to disk, such as in a database, as
// opposed to decoding blocks from the wire.
func (msg *MsgBlock) LatDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	buf := binarySerializer.Borrow()
	defer binarySerializer.Return(buf)

	err := msg.MsgHeader.LatDecode(r, pver, buf)
	if err != nil {
		return err
	}
	...
	if txCount > maxTxPerBlock {
		...
		return messageError("MsgBlock.LatDecode", str)
	}
	...
	for i := uint64(0); i < txCount; i++ {
		tx := MsgTx{}
		err := tx.latDecode(r, pver, enc, buf, scriptBuf[:])
		if err != nil {
			return err
		}
		msg.Transactions = append(msg.Transactions, &tx)
...
// Version field in the block.  This function differs from LatDecode in that
// LatDecode decodes from the wire protocol as it was sent across the
...
func (msg *MsgBlock) Deserialize(r io.Reader) error {
	...
	// a result, make use of LatDecode.
	//
	// Passing an encoding type of WitnessEncoding to LatEncode for the
	...
	return msg.LatDecode(r, 0, WitnessEncoding)
}
...
func (msg *MsgBlock) DeserializeNoWitness(r io.Reader) error {
	return msg.LatDecode(r, 0, BaseEncoding)
}
...
func (msg *MsgBlock) DeserializeTxLoc(r *bytes.Buffer) ([]TxLoc, error) {
	...
	err := msg.MsgHeader.LatDecode(r, 0, buf)
	if err != nil {
		return nil, err
	}
	...
	for i := uint64(0); i < txCount; i++ {
		...
		tx := MsgTx{}
		err := tx.latDecode(r, 0, WitnessEncoding, buf, scriptBuf[:])
		if err != nil {
			return nil, err
		}
		msg.Transactions = append(msg.Transactions, &tx)
...
// LatEncode encodes the receiver to w using the wire protocol encoding.
// This is part of the Message interface implementation.
// See Serialize for encoding blocks to be stored to disk, such as in a
// database, as opposed to encoding blocks for the wire.
func (msg *MsgBlock) LatEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	buf := binarySerializer.Borrow()
	defer binarySerializer.Return(buf)

	err := msg.MsgHeader.LatEncode(w, pver, buf)
	if err != nil {
		return err
	}
	...
	for _, tx := range msg.Transactions {
		err = tx.latEncode(w, pver, enc, buf)
		if err != nil {
			return err
		}
	}
...
// This function differs from LatEncode in that LatEncode encodes the block to
...
func (msg *MsgBlock) Serialize(w io.Writer) error {
	// At the current time, there is no difference between the wire encoding
	// at protocol version 0 and the stable long-term storage format.  As
	// a result, make use of LatEncode.
	//
	...
	// serialization structure defined in BIP0141.
	return msg.LatEncode(w, 0, WitnessEncoding)
}
...
func (msg *MsgBlock) SerializeNoWitness(w io.Writer) error {
	return msg.LatEncode(w, 0, BaseEncoding)
}
