...
func (msg *MsgTx) LatDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	err := msg.latDecode(r, pver, enc, buf, sbuf[:])
	return err
}

func (msg *MsgTx) latDecode(r io.Reader, pver uint32, enc MessageEncoding,
	buf, sbuf []byte) error {
	...
	if count == TxFlagMarker && enc == WitnessEncoding {
		...
		if flag[0] != WitnessFlag {
			...
			return messageError("MsgTx.LatDecode", str)
		}
...
// the Version field in the transaction.  This function differs from LatDecode
// in that LatDecode decodes from the wire protocol as it was sent
...
func (msg *MsgTx) LatEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	buf := binarySerializer.Borrow()
	defer binarySerializer.Return(buf)

	err := msg.latEncode(w, pver, enc, buf)
	return err
}

func (msg *MsgTx) latEncode(w io.Writer, pver uint32, enc MessageEncoding,
	buf []byte) error {
...
// the transaction.  This function differs from LatEncode in that LatEncode
...
func (msg *MsgTx) Serialize(w io.Writer) error {
	...
	//
	// Passing a encoding type of WitnessEncoding to LatEncode for MsgTx
