...
// Lattice network (magic) 4 bytes + command 12 bytes + payload length 4 bytes +
// checksum 4 bytes.
...
// Commands used in Lattice message headers which describe the type of message.
const (
	CmdVersion      = "version"
	CmdVerAck       = "verack"
	CmdGetAddr      = "getaddr"
...
const (
	...
	// for the Lattice wire protocol.
	BaseEncoding MessageEncoding = 1 << iota

	// WitnessEncoding encodes all messages other than transaction messages
	// using the default Lattice wire protocol specification. For transaction
	// messages, the new encoding format detailed in BIP0144 will be used.
	WitnessEncoding
)

// LatestEncoding is the most recently specified encoding for the Lattice wire
// protocol.
...
type Message interface {
	LatDecode(io.Reader, uint32, MessageEncoding) error
	LatEncode(io.Writer, uint32, MessageEncoding) error
	Command() string
...
func WriteMessageN(w io.Writer, msg Message, pver uint32, latnet LatticeNet) (int, error) {
	return WriteMessageWithEncodingN(w, msg, pver, latnet, BaseEncoding)
}
...
func WriteMessage(w io.Writer, msg Message, pver uint32, latnet LatticeNet) error {
	_, err := WriteMessageN(w, msg, pver, latnet)
	return err
}
...
func WriteV2MessageN(w io.Writer, msg Message, pver uint32,
	...
	var bw bytes.Buffer
	err := msg.LatEncode(&bw, pver, encoding)
	if err != nil {
...
func WriteMessageWithEncodingN(w io.Writer, msg Message, pver uint32,
	latnet LatticeNet, encoding MessageEncoding) (int, error) {
...
func ReadV2MessageN(plaintext []byte, pver uint32, enc MessageEncoding) (
	...
	err = msg.LatDecode(buf, pver, enc)
	if err != nil {
		return nil, nil, err
	}
...
func ReadMessageWithEncodingN(r io.Reader, pver uint32, latnet LatticeNet,
	...
	return readMessageWithEncodingNInternal(
		r, pver, hdr, latnet, enc, totalBytes,
	)
}
...
func readMessageWithEncodingNInternal(r io.Reader, pver uint32,
	hdr *messageHeader, latnet LatticeNet, enc MessageEncoding,
	...
	// MsgVersion LatDecode function requires it.
	pr := bytes.NewBuffer(payload)
	err = msg.LatDecode(pr, pver, enc)
	if err != nil {
		return totalBytes, nil, nil, err
	}
...
func ReadMessageN(r io.Reader, pver uint32, latnet LatticeNet) (int, Message, []byte, error) {
	return ReadMessageWithEncodingN(r, pver, latnet, BaseEncoding)
}
...
func ReadMessage(r io.Reader, pver uint32, latnet LatticeNet) (Message, []byte, error) {
	_, msg, buf, err := ReadMessageN(r, pver, latnet)
	return msg, buf, err
}
