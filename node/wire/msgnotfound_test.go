...
func TestNotFoundWire(t *testing.T) {
	...
	for i, test := range tests {
		...
		if err != nil {
			t.Errorf("LatEncode #%d error %v", i, err)
			continue
		}
		if !bytes.Equal(buf.Bytes(), test.buf) {
			t.Errorf("LatEncode #%d\n got: %s want: %s", i,
		...
		if err != nil {
			t.Errorf("LatDecode #%d error %v", i, err)
			continue
		}
		if !reflect.DeepEqual(&msg, test.out) {
			t.Errorf("LatDecode #%d\n got: %s want: %s", i,
...
func TestNotFoundWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		if reflect.TypeOf(err) != reflect.TypeOf(test.writeErr) {
			t.Errorf("LatEncode #%d wrong error got: %v, want: %v",
				i, err, test.writeErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.writeErr {
				t.Errorf("LatEncode #%d wrong error got: %v, "+
		...
		if reflect.TypeOf(err) != reflect.TypeOf(test.readErr) {
			t.Errorf("LatDecode #%d wrong error got: %v, want: %v",
				i, err, test.readErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.readErr {
				t.Errorf("LatDecode #%d wrong error got: %v, "+
