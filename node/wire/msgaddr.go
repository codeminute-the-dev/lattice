...
// addr message (MsgAddr).
const MaxAddrPerMsg = 1000

// MsgAddr implements the Message interface and represents a
...
func (msg *MsgAddr) LatDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxAddrPerMsg {
		...
		return messageError("MsgAddr.LatDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgAddr) LatEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	count := len(msg.AddrList)
	if count > MaxAddrPerMsg {
		str := fmt.Sprintf("too many addresses for message "+
			"[count %v, max %v]", count, MaxAddrPerMsg)
		return messageError("MsgAddr.LatEncode", str)
	}
...
// NewMsgAddr returns a new addr message that conforms to the
