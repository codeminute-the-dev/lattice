...
/*
Package wire implements the Lattice wire protocol.

For the complete details of the protocol, see the wiki entry
...
supported wire messages to and from the network.  This package does not deal
with the specifics of message handling such as what to do when a message is
received.  This provides the caller with a high level of flexibility.

# Message Overview

The protocol consists of exchanging messages between peers.  Each
message is preceded by a header which identifies information about it such as
which network it is a part of, its type, how big it is, and a checksum
to verify validity.  All encoding and decoding of message headers is handled by
this package.

To accomplish this, there is a generic interface for wire messages named
...
of the currently supported wire messages are provided.  For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using the protocol encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the wire messages are intended
to interact with one another.  As stated above, these interactions are not
directly handled by this package.  For more in-depth details about the
appropriate interactions, see the protocol wiki entry at
...
and write wire messages.  The following sections provide a quick overview of
...
The network identifier is a magic number which is used to identify the start of
a message and which network the message applies to.  This package provides
...
As discussed in the message overview section, this package reads
and writes wire messages using a generic interface named Message.  In
...
switch or type assertion.  An example of a type switch follows:
	...
	case *wire.MsgBlock:
		...
		fmt.Printf("Number of tx in block: %v", len(msg.Transactions))
	}

# Reading Messages

In order to unmarshall wire messages from the network, use the ReadMessage
function.  It accepts any io.Reader, but typically this will be a net.Conn to
a remote peer node.  Example syntax is:

	// Reads and validates the next wire message from conn using the
	// protocol version pver and the network latnet.  The returns
	// are a wire.Message, a []byte which contains the unmarshalled
	// raw payload, and a possible error.
	msg, rawPayload, err := wire.ReadMessage(conn, pver, latnet)
	if err != nil {
...
In order to marshall wire messages to the network, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote peer node.  Example syntax to request addresses
from a remote peer is:

	// Create a new getaddr wire message.
	msg := wire.NewMsgGetAddr()

	// Writes a wire message msg to conn using the protocol version
	// pver, and the network latnet.  The return is a possible
	// error.
	err := wire.WriteMessage(conn, msg, pver, latnet)
	if err != nil {
		// Log and handle the error
	}
