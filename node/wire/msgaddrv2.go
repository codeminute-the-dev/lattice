...
// LatDecode decodes r using the wire protocol into a MsgAddrV2.
func (m *MsgAddrV2) LatDecode(r io.Reader, pver uint32,
	...
	if count > MaxV2AddrPerMsg {
		...
		return messageError("MsgAddrV2.LatDecode", str)
	}
...
// LatEncode encodes the MsgAddrV2 into a writer w.
func (m *MsgAddrV2) LatEncode(w io.Writer, pver uint32,
	enc MessageEncoding) error {
	...
	if count > MaxV2AddrPerMsg {
		...
		return messageError("MsgAddrV2.LatEncode", str)
	}
...
// NewMsgAddrV2 returns a new addrv2 message that conforms to the
// Message interface.
