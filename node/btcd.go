...
import (
	...
	"github.com/codeminute-the-dev/lattice/node/blockchain/indexers"
	"github.com/codeminute-the-dev/lattice/node/database"
	"github.com/codeminute-the-dev/lattice/node/limits"
	"github.com/codeminute-the-dev/lattice/node/ossec"
)
...
// winServiceMain is only invoked on Windows.  It detects when latticed is running
// as a service and reacts accordingly.
var winServiceMain func() (bool, error)

// latticedMain is the real main function for latticed.  It is necessary to work around
// the fact that deferred functions do not run when os.Exit() is called.  The
// optional serverChan parameter is mainly used by the service code to be
// notified with the server once it is setup so it can gracefully stop it when
// requested from the service control manager.
func latticedMain(serverChan chan<- *server) error {
	...
	defer latdLog.Info("Shutdown complete")

	// Show version at startup.
	latdLog.Infof("Version %s", version())

	// Enable http profiling server if requested.
	if cfg.Profile != "" {
		go func() {
			listenAddr := net.JoinHostPort("", cfg.Profile)
			latdLog.Infof("Profile server listening on %s", listenAddr)
			profileRedirect := http.RedirectHandler("/debug/pprof",
				http.StatusSeeOther)
			http.Handle("/", profileRedirect)
			latdLog.Errorf("%v", http.ListenAndServe(listenAddr, nil))
		}()
	}
	...
	if cfg.CPUProfile != "" {
		...
		if err != nil {
			latdLog.Errorf("Unable to create cpu profile: %v", err)
			return err
		}
	...
	if cfg.MemoryProfile != "" {
		...
		if err != nil {
			latdLog.Errorf("Unable to create memory profile: %v", err)
			return err
		}
		defer f.Close()
	...
	if cfg.TraceProfile != "" {
		...
		if err != nil {
			latdLog.Errorf("Unable to create execution trace: %v", err)
			return err
		}
		trace.Start(f)
		defer f.Close()
	...
	db, err := loadBlockDB()
	if err != nil {
		latdLog.Errorf("%v", err)
		return err
	}
	defer func() {
		// Ensure the database is sync'd and closed on shutdown.
		latdLog.Infof("Gracefully shutting down the database...")
		db.Close()
	}()
	...
	if err != nil {
		...
		latdLog.Errorf("Unable to start server on %v: %v",
			cfg.Listeners, err)
		return err
	}
	defer func() {
		latdLog.Infof("Gracefully shutting down the server...")
		server.Stop()
...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		latdLog.Infof("Removing regression test database from '%s'", dbPath)
		if fi.IsDir() {
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		latdLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		latdLog.Infof("Creating block database in memory.")
	...
	latdLog.Infof("Loading block database from '%s'", dbPath)
	...
	latdLog.Info("Block database loaded")
	return db, nil
}
...
func main() {
	...
	if err := latticedMain(nil); err != nil {
		os.Exit(1)
	}
}
