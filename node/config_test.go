...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-latticed.conf")

	// Setup a temporary directory
	tmpDir := t.TempDir()
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of latticed binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-latticed.conf")
