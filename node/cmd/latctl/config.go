...
var (
	latticedHomeDir       = btcutil.AppDataDir("latticed", false)
	latctlHomeDir         = btcutil.AppDataDir("latctl", false)
	latwalletHomeDir      = btcutil.AppDataDir("latwallet", false)
	defaultConfigFile     = filepath.Join(latctlHomeDir, "latctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(latticedHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(latwalletHomeDir, "rpc.cert")
)
...
// config defines the configuration options for latctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(latctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		// Use config file for RPC server to create default latctl config
		var serverConfigPath string
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(latwalletHomeDir, "latwallet.conf")
		} else {
			serverConfigPath = filepath.Join(latticedHomeDir, "latticed.conf")
		}
...
// For this it tries to read the config file for the RPC server (either latticed or
// latwallet), and extract the RPC user and password from it.
