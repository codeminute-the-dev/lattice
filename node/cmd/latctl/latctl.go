...
// run executes latctl, returning an error on failure.  main is responsible for
...
// writeHelp writes the go-flags generated help plus latctl's stdin convention.
...
func newRPCClient(cfg *config) (*rpcclient.Client, error) {
	...
	return rpcclient.New(&rpcclient.ConnConfig{
		...
		HTTPPostMode:  true,
		// latctl is a one-shot CLI; fail fast instead of retrying.
		HTTPPostTries: 1,
	}, nil)
}
