...
// CalcBlockSubsidy lives in emission.go, alongside the rest of the
// sawtooth reward schedule.
...
func CheckTransactionSanity(tx *btcutil.Tx) error {
	...
	// as a cell.  One lattice is a quantity of cell as defined by the
	// CellPerLatt constant.
	//
	// Additionally, ensure all outputs use one of Lattice's supported script
	...
	for i, txOut := range msgTx.TxOut {
		...
		if cell > btcutil.MaxCell {
			str := fmt.Sprintf("transaction output value is "+
				"higher than max allowed value: %v > %v ",
				cell, btcutil.MaxCell)
			return ruleError(ErrBadTxOutValue, str)
		}

		// Two's complement int64 overflow guarantees that any overflow
		// is detected and reported.  This is impossible for Lattice, but
		...
		if totalCell > btcutil.MaxCell {
			str := fmt.Sprintf("total value of all transaction "+
				"outputs is %v which is higher than max "+
				"allowed value of %v", totalCell,
				btcutil.MaxCell)
			return ruleError(ErrBadTxOutValue, str)
		}
	}
...
func CheckTransactionInputs(tx *btcutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		// a transaction are in a unit value known as a cell.  One
		// lattice is a quantity of cell as defined by the
		// CellPerLatt constant.
		originTxCell := utxo.Amount()
		if originTxCell < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", btcutil.Amount(originTxCell))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxCell > btcutil.MaxCell {
			str := fmt.Sprintf("transaction output value is "+
				"higher than max allowed value: %v > %v ",
				btcutil.Amount(originTxCell),
				btcutil.MaxCell)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		lastCellIn := totalCellIn
		totalCellIn += originTxCell
		if totalCellIn < lastCellIn ||
			totalCellIn > btcutil.MaxCell {
			str := fmt.Sprintf("total value of all transaction "+
				"inputs is %v which is higher than max "+
				"allowed value of %v", totalCellIn,
				btcutil.MaxCell)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
	}
	...
	var totalCellOut int64
	for _, txOut := range tx.MsgTx().TxOut {
		totalCellOut += txOut.Value
	}

	// Ensure the transaction does not spend more than its inputs.
	if totalCellIn < totalCellOut {
		str := fmt.Sprintf("total value of all transaction inputs for "+
			"transaction %v is %v which is less than the amount "+
			"spent of %v", tx.Hash(), totalCellIn, totalCellOut)
	...
	txFeeInCell := totalCellIn - totalCellOut
	return txFeeInCell, nil
}
...
func (b *BlockChain) checkConnectBlock(node *blockNode, block *btcutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
	...
	expectedCellOut := CalcBlockSubsidy(node.height, b.chainParams) +
		totalFees
	if totalCellOut > expectedCellOut {
		str := fmt.Sprintf("coinbase transaction for block pays %v "+
			"which is more than expected value of %v",
			totalCellOut, expectedCellOut)
	...
	// All script validation behaviors are unconditional in Lattice.
