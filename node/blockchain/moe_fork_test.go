...
func moeSimNetParams() chaincfg.Params {
	...
	// Exercise the V1/V2 cutover in isolation. Lattice ships with every
	// proof-rule fork active from height 1, so the later forks have to be
	// switched off explicitly here or they would activate before the
	// cutover this test is about (and trip blockchain.New's ordering
	// assertion).
	params.SaltedSeedForkHeight = 0
	params.DenseOnlyForkHeight = 0
	params.RankPenaltyForkHeight = 0
	return params
}
