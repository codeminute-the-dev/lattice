...
type SendFromCmd struct {
	...
	ToAddress    string
	Amount       float64 // In LATT
	FeeRatePerKb float64 // Required: fee rate in LATT/KB
	MinConf      *int    `jsonrpcdefault:"1"`
	Comment      *string
	CommentTo    *string
}
...
type RawTxWitnessInput struct {
	...
	Amount        *float64 `json:"amount,omitempty"` // In LATT
}
...
func NewPsbtOutput(address string, amount btcutil.Amount) PsbtOutput {
	return PsbtOutput{address: amount.ToLATT()}
}
