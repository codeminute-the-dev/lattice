...
// field since it requires wallet access, which is outside the scope of latticed.
...
type TestMempoolAcceptFees struct {
	// Base is the transaction fee in LATT.
	Base float64 `json:"base"`

	// EffectiveFeeRate specifies the effective feerate in LATT per KvB. May
...
// GetNextResetResult models the data from the getnextreset command.
//
// Lattice's block reward decays smoothly within an emission epoch and then
// resets to its starting value. Every field derived from the schedule is
// computed from hardcoded consensus constants, so all nodes report identical
// values for a given height. Only the two "estimated" time fields depend on an
// assumption — that future blocks arrive at the target interval.
type GetNextResetResult struct {
	// Height is the current best block height.
	Height int32 `json:"height"`

	// Epoch is the zero-based emission epoch containing Height.
	Epoch int64 `json:"epoch"`

	// EpochBlocks is the fixed number of blocks in every epoch.
	EpochBlocks int32 `json:"epochblocks"`

	// HeightInEpoch is the 1-based position of Height within its epoch.
	HeightInEpoch int64 `json:"heightinepoch"`

	// EpochStartHeight is the height at which the current epoch began.
	EpochStartHeight int32 `json:"epochstartheight"`

	// NextResetHeight is the exact height at which the reward next resets.
	NextResetHeight int32 `json:"nextresetheight"`

	// BlocksUntilReset is the exact number of blocks remaining.
	BlocksUntilReset int32 `json:"blocksuntilreset"`

	// EstimatedSecondsUntilReset assumes the target block time.
	EstimatedSecondsUntilReset int64 `json:"estimatedsecondsuntilreset"`

	// EstimatedResetTime is the estimated Unix time of the reset.
	EstimatedResetTime int64 `json:"estimatedresettime"`

	// BlocksSinceLastReset is how far into the epoch the chain has come.
	BlocksSinceLastReset int64 `json:"blockssincelastreset"`

	// EstimatedSecondsSinceLastReset assumes the target block time.
	EstimatedSecondsSinceLastReset int64 `json:"estimatedsecondssincelastreset"`

	// CurrentReward is the reward of the block at Height, in LATT.
	CurrentReward float64 `json:"currentreward"`

	// NextReward is the reward of the following block, in LATT. At a reset
	// boundary this jumps back up to InitialReward.
	NextReward float64 `json:"nextreward"`

	// InitialReward is the reward every epoch starts at, in LATT.
	InitialReward float64 `json:"initialreward"`

	// MintedThisEpoch is the supply minted since the last reset, in LATT.
	MintedThisEpoch float64 `json:"mintedthisepoch"`

	// ResetThreshold is the per-epoch emission that triggers a reset, in LATT.
	ResetThreshold float64 `json:"resetthreshold"`

	// TotalSupply is all supply minted through Height, in LATT. There is no
	// maximum supply; this grows without bound as epochs repeat.
	TotalSupply float64 `json:"totalsupply"`

	// TargetBlockSeconds is the network's target block interval.
	TargetBlockSeconds float64 `json:"targetblockseconds"`
}
