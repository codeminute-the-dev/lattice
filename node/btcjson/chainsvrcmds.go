...
// LATTPerkvB is the units used to represent transaction fees.
// This unit represents the fee in LATT for a transaction size of 1 kB.
type LATTPerkvB = float64
...
type CreateRawTransactionCmd struct {
	...
	Amounts  map[string]float64 `jsonrpcusage:"{\"address\":amount,...}"` // In LATT
	LockTime *int64
}
...
//
// Amounts are in LATT. Passing in nil and the empty slice as inputs is equivalent,
...
type FundRawTransactionOpts struct {
	...
	FeeRate                *LATTPerkvB           `json:"feeRate,omitempty"` // LATT/kB
...
// GetNextResetCmd defines the getnextreset JSON-RPC command, which reports
// where the chain sits in Lattice's sawtooth emission schedule and when the
// block reward next resets to its starting value.
type GetNextResetCmd struct{}

// NewGetNextResetCmd returns a new instance which can be used to issue a
// getnextreset JSON-RPC command.
func NewGetNextResetCmd() *GetNextResetCmd {
	return &GetNextResetCmd{}
}
...
// maxFeeRate is the maximum fee rate for the transaction in LATT/kvB.
//
// A 0 maxFeeRate indicates that a maximum fee rate won't be enforced.
func NewSendRawTransactionCmdWithMaxFeeRate(hexTx string, maxFeeRate LATTPerkvB) *SendRawTransactionCmd {
	return &SendRawTransactionCmd{
		HexTx: hexTx,
		FeeSetting: &AllowHighFeesOrMaxFeeRate{
			Value: &maxFeeRate,
...
type TestMempoolAcceptCmd struct {
	...
	// value, expressed in LATT/kvB, optional, default="0.10".
	MaxFeeRate LATTPerkvB `json:"omitempty"`
}
...
func NewTestMempoolAcceptCmd(rawTxns []string,
	maxFeeRate LATTPerkvB) *TestMempoolAcceptCmd {
...
func init() {
	...
	MustRegisterCmd("getnextreset", (*GetNextResetCmd)(nil), flags)
