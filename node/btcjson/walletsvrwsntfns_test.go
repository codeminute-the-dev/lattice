...
func TestWalletSvrWsNtfns(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "btcdconnected",
			newNtfn: func() (interface{}, error) {
				return btcjson.NewCmd("latticedconnected", true)
			},
			staticNtfn: func() interface{} {
				return btcjson.NewLatticedConnectedNtfn(true)
			},
			marshalled: `{"jsonrpc":"1.0","method":"latticedconnected","params":[true],"id":null}`,
			unmarshalled: &btcjson.LatticedConnectedNtfn{
				Connected: true,
			},
		},
		{
