...
func (c *Client) BackendVersion() (BackendVersion, error) {
	...
	// connected to a latticed backend if it does exist.
	info, err := c.GetInfo()

	switch err := err.(type) {
	// Detected latticed backend.
	case nil:
		log.Debugf("Detected latticed backend (version %v)", info.Version)
		c.backendVersion = LatticedVersion(info.Version)
		return c.backendVersion, nil
	...
	case *btcjson.RPCError:
		if err.Code != btcjson.ErrRPCMethodNotFound.Code {
			return nil, fmt.Errorf("unable to detect latticed version: "+
				"%v", err)
		}

	default:
		return nil, fmt.Errorf("unable to detect latticed version: %v", err)
	}
	...
	// implementing the Lattice protocol), which exposes its version through
	// GetNetworkInfo.
...
// compatible with latticed. UDP is not supported because the node needs reliable
// connections.
