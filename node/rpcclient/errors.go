...
var LatticedErrMap = map[string]error{
	...
	//
	// NOTE: For latticed v0.24.2 and beyond, the error message is "already
...
// chain backend and map it to an defined error here. It uses the `LatticedErrMap`
// defined above, whose keys are latticed error strings and values are errors made
// from compatible fork error strings.
//
// NOTE: we assume neutrino shares the same error strings as latticed.
func MapRPCErr(rpcErr error) error {
	// Iterate the error map and find the matching error.
	for latticedErr, err := range LatticedErrMap {
