...
const (
	// defaultMaxFeeRate is the default maximum fee rate in LATT/kvB for
	// transaction broadcast.
	defaultMaxFeeRate btcjson.LATTPerkvB = 0.1
)
...
func (c *Client) CreateRawTransactionAsync(inputs []btcjson.TransactionInput,
	...
	for addr, amount := range amounts {
		convertedAmts[addr.String()] = amount.ToLATT()
	}
...
func (c *Client) SendRawTransactionAsync(tx *wire.MsgTx, allowHighFees bool) FutureSendRawTransactionResult {
	...
	var maxFeeRate btcjson.LATTPerkvB
	if !allowHighFees {
...
func (c *Client) TestMempoolAcceptAsync(txns []*wire.MsgTx,
	maxFeeRate btcjson.LATTPerkvB) FutureTestMempoolAcceptResult {
	...
	for _, tx := range txns {
		...
		// TODO(yy): add similar checks found in `LatDecode` to
		// `LatEncode` - atm it just serializes bytes without any
		...
		// once we have similar checks added in `LatEncode`.
		//
...
func (c *Client) TestMempoolAccept(txns []*wire.MsgTx,
	maxFeeRate btcjson.LATTPerkvB) ([]*btcjson.TestMempoolAcceptResult, error) {
