...
RPC server that uses a latticed-compatible JSON-RPC API.  This client has been
tested with latticed (https://github.com/codeminute-the-dev/lattice) and
Latwallet (https://github.com/codeminute-the-dev/lattice).

In addition to the compatible standard HTTP POST JSON-RPC API, latticed and
Latwallet provide a websocket interface that is more efficient than the standard
...
latticed or Latwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by latticed and
Latwallet only uses a single connection that remains open and allows
...
All notifications provided by latticed require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by latticed (and
Latwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that latticed intentionally separates the wallet
functionality into a separate process named Latwallet.  This means if you are
connected to the latticed RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, Latwallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
  ...
  - latticedwebsockets
    ...
    block count
  - latwalletwebsockets
    Connects to an Latwallet RPC server using TLS-secured websockets, registers
