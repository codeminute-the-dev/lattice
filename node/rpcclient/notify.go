...
type NotificationHandlers struct {
	...
	//
	// NOTE: This is a Lattice extension ported from
	...
	// result of a rescan request, due to how latticed may send various rescan
	...
	// OnLatticedConnected is invoked when a wallet connects or disconnects from
	// latticed.
	//
	// This will only be available when client is connected to a wallet
	// server such as Latwallet.
	OnLatticedConnected func(connected bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	// OnLatticedConnected
	case btcjson.LatticedConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		if c.ntfnHandlers.OnLatticedConnected == nil {
			return
		}

		connected, err := parseLatticedConnectedNtfnParams(ntfn.Params)
		if err != nil {
			log.Warnf("Received invalid latticed connected "+
				"notification: %v", err)
			return
		}

		c.ntfnHandlers.OnLatticedConnected(connected)
...
//
// NOTE: This is a latticed extension ported from github.com/decred/dcrrpcclient
...
// parseLatticedConnectedNtfnParams parses out the connection status of latticed
// and Latwallet from the parameters of a latticedconnected notification.
func parseLatticedConnectedNtfnParams(params []json.RawMessage) (bool, error) {
	if len(params) != 1 {
...
//
// NOTE: This is a latticed extension and requires a websocket connection.
