...
func TestAddresses(t *testing.T) {
	tests := []struct {
		...
		// Taproot address tests - these should pass
		{
			...
			addr:    "lat1paardr2nczq0rx5rqpfwnvpzm497zvux64y0f7wjgcs7xuuuh2nnq0gljv2",
			encoded: "lat1paardr2nczq0rx5rqpfwnvpzm497zvux64y0f7wjgcs7xuuuh2nnq0gljv2",
			valid:   true,
		...
		},
		{
			...
			addr:    "tlat1paardr2nczq0rx5rqpfwnvpzm497zvux64y0f7wjgcs7xuuuh2nnqy8mvnl",
			encoded: "tlat1paardr2nczq0rx5rqpfwnvpzm497zvux64y0f7wjgcs7xuuuh2nnqy8mvnl",
			valid:   true,
			result: btcutil.TstAddressTaproot(
				1, [32]byte{
					0xef, 0x46, 0xd1, 0xaa, 0x78, 0x10, 0x1e, 0x33,
		...
		// Legacy addresses - these should fail
		{
			...
			addr:  "lat1qw508d6qejxtdg4y5r3zarvary0c5xw7kxlppwd",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
		{
			name:  "legacy segwit v0 p2wsh",
			addr:  "lat1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qenuy5r",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
		{
			name:  "invalid witness version",
			addr:  "lat1sqqqsyqcyq5rqwzqfpg9scrgwpugpzysn7yxe66",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
		{
			name:  "invalid taproot program length",
			addr:  "lat1pqypqxlmwv74",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
	}
...
func TestEncodeDecodeAddressStringer(t *testing.T) {
	...
	// Test stringer
	expectedAddr := "lat1paardr2nczq0rx5rqpfwnvpzm497zvux64y0f7wjgcs7xuuuh2nnq0gljv2"
...
func TestDecodeAddressRejectsLegacy(t *testing.T) {
	tests := []struct {
		...
		{
			name: "P2WPKH_mainnet",
			addr: "lat1qqqqsyqcyq5rqwzqfpg9scrgwpugpzysnl58psv",
			desc: "P2WPKH (witness v0, 20 bytes) mainnet",
		},
		{
			name: "P2WPKH_testnet",
			addr: "tlat1qqqqsyqcyq5rqwzqfpg9scrgwpugpzysngxygrx",
			desc: "P2WPKH (witness v0, 20 bytes) testnet",
		},
		{
			name: "P2WSH_mainnet",
			addr: "lat1qqqqsyqcyq5rqwzqfpg9scrgwpugpzysnzs23v9ccrydpk8qarc0srg7dnv",
			desc: "P2WSH (witness v0, 32 bytes) mainnet",
		},
		{
			name: "P2WSH_testnet",
			addr: "tlat1qqqqsyqcyq5rqwzqfpg9scrgwpugpzysnzs23v9ccrydpk8qarc0sg86nve",
