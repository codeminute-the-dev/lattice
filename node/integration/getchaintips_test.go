...
func TestGetChainTips(t *testing.T) {
	...
	expectedChainTips := []*btcjson.GetChainTipsResult{
		{
			Height: 0,
			// This harness runs on simnet, so the tip is simnet's genesis.
			// Pearl's simnet and regtest genesis blocks were byte-identical,
			// which let this read the wrong params and still pass; Lattice
			// gives every network a distinct genesis.
			Hash:      netParams.GenesisHash.String(),
			BranchLen: 0,
			Status:    "active",
		},
	}
