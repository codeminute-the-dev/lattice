...
import (
	...
	rpc "github.com/codeminute-the-dev/lattice/node/rpcclient"
)

// nodeConfig contains all the args, and data required to launch a latticed process
...
func newConfig(nodeDir, certFile, keyFile string, extra []string,
	...
	var latticedPath string
	if customExePath != "" {
		latticedPath = customExePath
	} else {
		var err error
		latticedPath, err = latticedExecutablePath()
		if err != nil {
			latticedPath = "latticed"
		}
	}
...
// arguments returns an array of arguments that be used to launch the latticed
// process.
...
// command returns the exec.Cmd which will be used to start the latticed process.
...
// to the latticed process that is launched via Start().
...
// as the base for the log and data directories for latticed.
...
// start creates a new latticed process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.dataDir, "latticed.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running latticed process process, and waits until it exits
...
// shutdown terminates the running latticed process, and cleans up all
