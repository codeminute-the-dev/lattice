...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt btcutil.Amount) *chainhash.Hash {
		...
		// Next, send amt LATT to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(btcutil.Amount(5 * btcutil.CellPerLatt))
	...
	// Lattice pays 2 LATT per block, so 25 mature outputs total roughly
	// 50 LATT. Spends have to fit inside that, unlike under Pearl's
	// far larger per-block reward.
	txid = genSpend(btcutil.Amount(10 * btcutil.CellPerLatt))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(btcutil.CellPerLatt, pkScript)
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	if int64(walletBalance) != expectedBalance {
		t.Fatalf("wallet balance incorrect: expected %v Lattice, got %v Lattice",
	...
	// The original wallet should now have a balance of 0 Lattice as its entire
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := btcutil.Amount(5 * btcutil.CellPerLatt)
	...
	// The current wallet balance should now be at least 5 LATT less
...
func TestHarness(t *testing.T) {
	...
	if int64(harnessBalance) != expectedBalance {
		t.Fatalf("expected wallet balance of %v Lattice instead have %v Lattice",
