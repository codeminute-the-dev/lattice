...
// Harness fully encapsulates an active latticed process to provide a unified
// platform for creating rpc driven integration tests involving latticed. The
// active latticed node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active latticed process is fully
...
// used. If a custom latticed executable is specified, it will be used to start the
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the latticed node itself. This spawns a new process which will be
	// managed
	...
	// Ensure latticed properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created latticed
...
// expressed in cells-per-byte. The transaction being created can optionally
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "latticed", "rpctest")
