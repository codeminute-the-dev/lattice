...
func testScripts(t *testing.T, tests [][]interface{}, useSigCache bool) {
	...
	for i, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			// Lattice only supports Taproot (SegWit v1). Each Bitcoin
			...
			//   check succeeds because Lattice doesn't evaluate the
			...
			if resultStr != "OK" {
				// Expected to fail. On Lattice, these should still
				// fail unless they are P2SH scripts that now pass
				// because Lattice doesn't unwrap the redeem script.
				if execErr == nil {
...
func TestTxInvalidTests(t *testing.T) {
	...
	for i, test := range tests {
		...
		t.Run(fmt.Sprintf("test_%d", i), func(t *testing.T) {
			...
			//
			// Exception: P2SH transactions may pass because Lattice
...
func TestTxValidTests(t *testing.T) {
	...
	for i, test := range tests {
		...
		t.Run(fmt.Sprintf("test_%d", i), func(t *testing.T) {
			...
			// Lattice only supports Taproot (SegWit v1). These Bitcoin Core
			// "valid" test vectors are deterministic on Lattice: taproot
...
// involving legacy/compat/removed-feature scripts behaves correctly on Lattice.
//
// Success path: any outcome is accepted. These test vectors may exercise
// features Lattice has removed (OP_SUCCESS, unknown leaf versions, etc.),
...
// fail on Bitcoin should also fail on Lattice (possibly for a different reason).
func executeRejectedTaprootRefTest(t *testing.T, testCase taprootJsonTest) {
	...
	if testCase.Failure != nil {
		...
		// P2SH scripts: the outer HASH160 check passes because Lattice
...
func TestTaprootReferenceTests(t *testing.T) {
	...
	err := filepath.Walk(filePath, func(path string, info fs.FileInfo, walkErr error) error {
		...
		// removed in Lattice (unknown leaf versions, OP_SUCCESS,
