...
const (
	// svcName is the name of the latticed service.
	svcName = "latticedsvc"
	...
	svcDisplayName = "Lattice Node Service"

	// svcDesc is the description of the service.
	svcDesc = "Downloads and stays synchronized with the Lattice blockchain " +
...
// logServiceStartOfDay logs information about latticed when the main server has
...
// latticedService houses the main service handler which handles all service
// updates and launching latticedMain.
type latticedService struct{}
...
// long-running latticedMain (which is the real meat of latticed), handles service
// change requests, and notifies the service control manager of changes.
func (s *latticedService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start latticedMain in a separate goroutine so the service can start
...
// installService attempts to install the latticed service.  Typically this should
...
// removeService attempts to uninstall the latticed service.  Typically this should
...
// startService attempts to start the latticed service.
...
func serviceMain() (bool, error) {
	...
	// needed to run latticed on Windows in CI environments like Travis.
	...
	err = svc.Run(svcName, &latticedService{})
	if err != nil {
