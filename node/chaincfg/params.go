...
var (
	...
	// simNetPowLimit is the highest proof of work value a Lattice block
	// can have for the simulation test network.
	simNetPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 233), bigOne)

	// sigNetPowLimit is the highest proof of work value a Lattice block can
	...
	// signet network. Lattice does not currently operate a signet.
	DefaultSignetDNSSeeds = []DNSSeed{}
)

const (
	// HDCoinTypeLattice is the BIP-44 coin type for Lattice mainnet.
	// Derived from ASCII "LATT": 808276 (0xC5554).
	HDCoinTypeLattice = 808276
...
// Params defines a Lattice network by its parameters.  These parameters may be
...
type Params struct {
	...
	// Emission is the block reward schedule: the hyperbolic decay curve and
	// the epoch reset that restarts it. See chaincfg/emission.go.
...
// MainNetParams defines the network parameters for the main Lattice network.
var MainNetParams = Params{
	Name:        "mainnet",
	Net:         wire.MainNet,
	DefaultPort: "44108",
	DNSSeeds: []DNSSeed{
		{"seeder1.lattice.codeminute.dev", false},
		{"seeder2.lattice.codeminute.dev", false},
		{"seeder3.lattice.codeminute.dev", false},
	},
	...
	// Lattice launches with no checkpoints. Pearl's were hashes of Pearl
	// blocks and could never appear on this chain. Add entries here as the
	// chain accumulates history, one per 10,000 blocks, once each is at
	// least CheckpointConfirmations deep.
	Checkpoints: nil,
	...
	// BIP 173.
	Bech32HRPSegwit: "lat", // always lat for main net
	...
	// BIP44 coin type used in the hierarchical deterministic path for
	// address generation.
	HDCoinType: HDCoinTypeLattice,
}

// RegressionNetParams defines the network parameters for the regression test
// Lattice network.  Not to be confused with the test Lattice network (version
...
var RegressionNetParams = Params{
	...
	// Human-readable part for Bech32 encoded segwit addresses, as defined in
	// BIP 173.
	Bech32HRPSegwit: "rlat", // always rlat for reg test net
...
// TestNetParams defines the network parameters for the test Lattice network.
...
var TestNetParams = Params{
	...
	DefaultPort: "44110",
	DNSSeeds: []DNSSeed{
		{"seeder1.internal.lattice.codeminute.dev", false},
		{"seeder2.internal.lattice.codeminute.dev", false},
		{"seeder3.internal.lattice.codeminute.dev", false},
	},
	...
	// Human-readable part for Bech32 encoded segwit addresses, as defined in
	// BIP 173.
	Bech32HRPSegwit: "tlat", // always tlat for test net
...
// TestNet2Params defines the network parameters for the Lattice test network v2,
...
var TestNet2Params = Params{
	...
	DefaultPort: "44112",
	DNSSeeds: []DNSSeed{
		{"seeder1.testnet.lattice.codeminute.dev", false},
		{"seeder2.testnet.lattice.codeminute.dev", false},
		{"seeder3.testnet.lattice.codeminute.dev", false},
	},
...
// SimNetParams defines the network parameters for the simulation test Lattice
...
var SimNetParams = Params{
	...
	// Human-readable part for Bech32 encoded segwit addresses, as defined in
	// BIP 173.
	Bech32HRPSegwit: "rlat", // always rlat for sim net
...
// Lattice network. Not to be confused with the regression test network, this
...
func CustomSignetParams(challenge []byte, dnsSeeds []DNSSeed) Params {
	...
	return Params{
		...
		// Lattice activates the newest proof rules from the first block on
		// every network; there is no pre-fork history to remain compatible with.
		MoEForkHeight:         1,
		SaltedSeedForkHeight:  1,
		DenseOnlyForkHeight:   1,
		RankPenaltyForkHeight: 1,
		...
		// Human-readable part for Bech32 encoded segwit addresses, as defined in
		// BIP 173.
		Bech32HRPSegwit: "tlat", // always tlat for sig net
...
var (
	// ErrDuplicateNet describes an error where the parameters for a Lattice
	// network could not be set due to the network already being a standard
	// network or previously-registered into this package.
	ErrDuplicateNet = errors.New("duplicate Lattice network")
...
var (
	registeredNets       = make(map[wire.LatticeNet]struct{})
...
// Register registers the network parameters for a Lattice network.  This may
