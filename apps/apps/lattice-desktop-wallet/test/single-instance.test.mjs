...
const STARTUP_BANNER = 'Lattice Desktop Wallet starting...';
...
test('duplicate launch exits immediately and leaves the primary instance running', async (t) => {
  ...
  const configHome = mkdtempSync(join(tmpdir(), 'lattice-wallet-test-'));
  const mainLog = join(configHome, '@lattice', 'lattice-desktop-wallet', 'logs', 'main.log');
