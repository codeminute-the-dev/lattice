...
const binaryNameMap: Record<string, Record<string, string>> = {
  win32: {
    x64: 'latwallet-windows-x64.exe',
    ia32: 'latwallet-windows-ia32.exe',
  },
  linux: {
    x64: 'latwallet-linux-x64',
    arm64: 'latwallet-linux-arm64',
  },
  darwin: {
    x64: 'latwallet-darwin-x64',
    arm64: 'latwallet-darwin-arm64',
  },
};
...
class WalletProcess {
  ...
  getWalletArgs(): string[] {
    ...
    // (seeder1/2/3.lattice.codeminute.dev — see node/chaincfg/params.go).
  ...
  async killExistingWalletProcesses() {
    return new Promise<void>(resolve => {
      ...
      lsofProcess.on('close', code => {
        // The shipped daemon binary is named `latwallet-<platform>-<arch>`, which
        // lsof reports (truncated) as `latwallet-...` in its COMMAND column. The
        // previous match string ('latticewall') never appears, so a wedged prior
        // daemon holding the RPC port was never reaped. Match on 'latwallet'.
        if (code === 0 && output.includes('latwallet')) {
          const lines = output.split('\n');
          const pids: string[] = [];

          for (const line of lines) {
            if (line.includes('latwallet')) {
