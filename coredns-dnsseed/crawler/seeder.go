...
// Seeder discovers Lattice peers and maintains an address book for DNS serving.
type Seeder struct {
	config *peer.Config
	logger *log.Logger
...
func NewSeeder(networkName string) (*Seeder, error) {
	...
	logger := log.New(sink, "lattice-seeder: ", log.Ldate|log.Ltime|log.Lshortfile|log.LUTC)
