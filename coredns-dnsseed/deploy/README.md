# Lattice DNS Seeder (CoreDNS Plugin)

A CoreDNS plugin that crawls the Lattice P2P network and serves peer IP addresses
...
- A running `latticed` node (regtest mode is simplest)
- Docker
...
```bash
latticed --regtest --listen 127.0.0.1:18444
```

2. Build the CoreDNS image from the repo root:

```bash
docker build -f coredns-dnsseed/deploy/Dockerfile -t lattice-seeder .
```
...
| ----------------- | ------- | ---------------------------------------- |
| `network`         | —       | Lattice network: mainnet, testnet, testnet2, regtest, signet, simnet |
