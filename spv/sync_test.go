...
import (
	...
	"github.com/codeminute-the-dev/lattice/node/integration/rpctest"
...
func testStartRescan(harness *neutrinoHarness, t *testing.T) {
	...
	authTx1, err := txauthor.NewUnsignedTransaction(
		...
		// Fee rate is cells per kilobyte
		1024000,
		inSrc(*tx1),
...
func TestNeutrinoSync(t *testing.T) {
	...
	// Create a latticed SimNet node and generate 800 blocks
	h1, err := rpctest.New(
	...
	// Create a second latticed SimNet node
	h2, err := rpctest.New(
		&chaincfg.SimNetParams, nil, []string{"--txindex"}, "",
	)
	require.NoError(err, "Couldn't create harness")
	...
	// Create a third latticed SimNet node and generate 1200 blocks
	h3, err := rpctest.New(
		&chaincfg.SimNetParams, nil, []string{"--txindex"}, "",
	)
	require.NoError(err, "Couldn't create harness")
