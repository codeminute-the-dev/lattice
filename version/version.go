...
// Package version provides the canonical version for all Lattice binaries.
...
const (
	// Lattice's version numbering starts fresh at 0.1.0 rather than
	// continuing Pearl's. The codebase is inherited, but the chain,
	// its genesis, and its consensus parameters are not, so carrying
	// Pearl's version forward would imply a compatibility that does
	// not exist.
	Major uint = 0
	Minor uint = 1
	Patch uint = 0
...
// -ldflags "-X github.com/codeminute-the-dev/lattice/version.Build=<value>".
var Build string
