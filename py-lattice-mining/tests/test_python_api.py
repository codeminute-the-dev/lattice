"""
Tests for the lattice_mining Python API (PyO3 bindings).
"""

import base64

import lattice_mining
import pytest
...
# --- Helpers ---
def create_test_block_header(nbits: int = DEFAULT_NBITS) -> lattice_mining.IncompleteBlockHeader:
    return lattice_mining.IncompleteBlockHeader(
        version=0,
        prev_block=b"\x00" * 32,
...
) -> lattice_mining.MiningConfiguration:
    rows_pattern = lattice_mining.PeriodicPattern.from_list(ROWS_PATTERN_LIST)
    cols_pattern = lattice_mining.PeriodicPattern.from_list(COLS_PATTERN_LIST)
    moe = None if e is None else lattice_mining.MoEConfig(e=e, top_k=top_k)
    return lattice_mining.MiningConfiguration(
        common_dim=k,
        rank=rank,
        mma_type=lattice_mining.MMAType.Int7xInt7ToInt32,
...
def generate_moe_plain_proof(
    ...
    return lattice_mining.mine_moe(
        m,
        n,
        k,
        block_header,
...
def prove_and_verify(
    block_header: lattice_mining.IncompleteBlockHeader,
    plain_proof: lattice_mining.PlainProof,
    *,
    expect_valid: bool = True,
) -> tuple[lattice_mining.ZKProof, bool, str]:
    """Generate a V3 ZK proof and verify it. Asserts the expected outcome."""
    proof = lattice_mining.generate_proof_v3(block_header, plain_proof)
    is_valid, message = lattice_mining.verify_proof_v3(block_header, proof)
    if expect_valid:
...
def moe_prove_and_verify(
    block_header: lattice_mining.IncompleteBlockHeader,
    moe_proof: lattice_mining.PlainProof,
    *,
    expect_valid: bool = True,
) -> tuple[lattice_mining.ZKProof, bool, str]:
    """Generate a V3 ZK proof from an MoE mining solution and verify it."""
    proof = lattice_mining.generate_proof_v3(block_header, moe_proof)
    is_valid, message = lattice_mining.verify_proof_v3(block_header, proof)
    if expect_valid:
...
class TestVerifyPlainProof:
    ...
    def test_valid_plain_proof(self):
        ...
        is_valid, message = lattice_mining.verify_plain_proof_v3(block_header, plain_proof)
...
class TestMoEMineProveVerify:
    ...
    def test_basic_moe(self):
        ...
        is_valid, msg = lattice_mining.verify_plain_proof_v3(block_header, moe_proof)
...
class TestMoESerialization:
    ...
    def test_moe_proof_round_trip(self):
        ...
        restored = lattice_mining.PlainProof.from_base64(encoded)

        is_valid_original, _ = lattice_mining.verify_plain_proof_v3(block_header, moe_proof)
        is_valid_restored, _ = lattice_mining.verify_plain_proof_v3(block_header, restored)
...
VALID_CERT_VERSIONS = [
    lattice_mining.CERT_VERSION_ZK_DENSE,
    lattice_mining.CERT_VERSION_ZK_MOE,
    lattice_mining.CERT_VERSION_ZK_V3,
]
...
def make_dummy_plain_proof(*, moe: bool) -> lattice_mining.PlainProof:
    """Lightweight (non-verifiable) PlainProof for eligibility checks only."""
    merkle_proof = lattice_mining.MerkleProof(
        total_leaves=1,
    ...
    matrix_proof = lattice_mining.MatrixMerkleProof(proof=merkle_proof, row_indices=[0])
    moe_params = (
        lattice_mining.MoEProofParams(
    ...
    return lattice_mining.PlainProof(
        m=_DUMMY_DIMENSION,
        n=_DUMMY_DIMENSION,
        k=_DUMMY_DIMENSION,
...
class TestLegacyV1Deserialization:
    ...
    def test_accepts_legacy_v1_blob(self):
        ...
        plain_proof = generate_plain_proof(
            m, n, k, block_header, cert_version=lattice_mining.CERT_VERSION_ZK_DENSE
        )
        ...
        restored = lattice_mining.PlainProof.from_base64(legacy_blob)
        assert restored.min_cert_version == lattice_mining.CERT_VERSION_ZK_DENSE
        is_valid, msg = lattice_mining.verify_plain_proof_v2(block_header, restored)
        assert is_valid, f"restored legacy proof failed verification: {msg}"

    def test_rejects_garbage(self):
        garbage = base64.b64encode(b"\xab" * 7).decode()
        with pytest.raises(ValueError):
            lattice_mining.PlainProof.from_base64(garbage)
...
class TestCertVersionEligibility:
    ...
    def test_min_cert_version_exposed(self):
        ...
        assert dense.min_cert_version == lattice_mining.CERT_VERSION_ZK_DENSE
        assert moe.min_cert_version == lattice_mining.CERT_VERSION_ZK_MOE

    def test_dense_proof_eligible_under_both_versions(self):
        dense = make_dummy_plain_proof(moe=False)
        for cert_version in VALID_CERT_VERSIONS:
            lattice_mining.check_cert_version_eligible(cert_version, dense)

    def test_moe_proof_rejected_before_crossover(self):
        moe = make_dummy_plain_proof(moe=True)
        lattice_mining.check_cert_version_eligible(lattice_mining.CERT_VERSION_ZK_MOE, moe)
        with pytest.raises(ValueError, match="crossover"):
            lattice_mining.check_cert_version_eligible(lattice_mining.CERT_VERSION_ZK_DENSE, moe)

    def test_unknown_versions_rejected(self):
        dense = make_dummy_plain_proof(moe=False)
        for version in (0, 4):
            with pytest.raises(ValueError, match="unknown certificate version"):
                lattice_mining.check_cert_version_eligible(version, dense)
...
class TestCertVersionDispatchers:
    ...
    def test_dense_proof_round_trip_under_both_versions(self, cert_version):
        ...
        is_valid, msg = lattice_mining.verify_plain_proof_for_cert_version(
            cert_version, block_header, plain_proof
        )
        assert is_valid, f"plain proof verification failed (cert v{cert_version}): {msg}"

        proof = lattice_mining.generate_proof_for_cert_version(
            cert_version, block_header, plain_proof
        )
        is_valid, msg = lattice_mining.verify_proof_for_cert_version(
    ...
    def test_moe_proof_rejected_for_v1_block(self):
        ...
        block_header = create_test_block_header()
        with pytest.raises(ValueError, match="crossover"):
            lattice_mining.generate_proof_for_cert_version(
                lattice_mining.CERT_VERSION_ZK_DENSE, block_header, moe
            )
...
class TestSaltedCertVersion:
    ...
    def test_salted_round_trip(self):
        ...
        is_valid, msg = lattice_mining.verify_plain_proof_for_cert_version(
            lattice_mining.CERT_VERSION_ZK_V3, block_header, plain_proof
        )
        ...
        is_valid, msg = lattice_mining.verify_proof_for_cert_version(
            lattice_mining.CERT_VERSION_ZK_V3, block_header, proof
        )
    ...
    def test_cross_derivation_rejected(self):
        ...
        legacy_proof = generate_plain_proof(
            m, n, k, block_header, cert_version=lattice_mining.CERT_VERSION_ZK_MOE
        )
        is_valid, _ = lattice_mining.verify_plain_proof_v3(block_header, legacy_proof)
        ...
        is_valid, _ = lattice_mining.verify_plain_proof_v2(block_header, salted_proof)
...
class TestV1NbitsOverride:
    ...
    def test_share_difficulty_override(self):
        ...
        is_valid, msg = lattice_mining.verify_plain_proof_v1(
        ...
        is_valid, _ = lattice_mining.verify_plain_proof_v1(
