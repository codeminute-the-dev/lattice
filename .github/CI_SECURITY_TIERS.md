...
- **`miner_ci.yml`** — Ruff/clang-format checks, Lattice Gateway + Miner Base pytest
- **`rust_ci.yml`** — lattice-blake3, zk-pow, py-lattice-mining (fmt, clippy, tests)
...
- **`integration_tests_ci.yml`** — Full integration tests (latticed + vLLM + miner)
- **`miner_heavy_ci.yml`** — Performance tests + vLLM Docker image build
- **`lattice-desktop-wallet.yml`** — Cross-platform Electron + Go builds
- **`blockchain_build.yml`** — Continuous per-commit build (on `push` to master +
  ...
  `latticed` images (`ghcr.io/.../latticed:sha-<commit>`, needs `packages: write`)
...
- **`lattice-desktop-wallet.yml`** release job — Wallet release (protected by `release` environment)