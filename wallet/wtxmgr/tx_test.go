...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	// Switch txout amount to 1 LATT.  Transaction store doesn't
...
func TestCoinbases(t *testing.T) {
	...
	// Balance should be 0 if the coinbase is immature, 50 LATT at and beyond
	// maturity.
	//
...
func TestOutputLocks(t *testing.T) {
	...
	coinbase := newCoinBase(
		btcutil.CellPerLatt, btcutil.CellPerLatt*2,
	)
	coinbaseHash := coinbase.TxHash()

	// One of the spends will be unconfirmed.
	const unconfirmedBalance = btcutil.CellPerLatt / 2
	...
	const confirmedBalance = btcutil.CellPerLatt
