...
func ExampleStore_Balance() {
	...
	// Insert a transaction which outputs 10 LATT unmined and mark the output
	// as a credit.
	...
	// Output:
	// 10 LATT, 0 LATT, 0 LATT
	// 10 LATT, 10 LATT, 0 LATT
	// 10 LATT, 10 LATT, 10 LATT
}
...
func ExampleStore_Rollback() {
	...
	err = walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		...
		// Insert a transaction which outputs 10 LATT in a block at height 100.
...
func Example_basicUsage() {
	...
	// Insert an unmined transaction that outputs 10 LATT to a wallet address
	// at output 0.
	...
	// outputs.  Mark the second one (5 LATT) as wallet change.
