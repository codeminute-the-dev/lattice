...
type sources struct {
	...
	creds   string // e.g. "flags", "latwallet.conf", "latwallet.conf (auto-provisioned)", "none found"
	tls     string
	connect string
}
...
func resolutionStory(cfg *config) string {
	...
	rows := [][3]string{
		...
		{"config", cfg.latwalletConfPath(), cfg.src.conf},
	...
	binValue, binNote := cfg.LatwalletBin, "not found"
	...
	rows = append(rows,
		...
		[3]string{"latwallet bin", binValue, binNote},
	)
