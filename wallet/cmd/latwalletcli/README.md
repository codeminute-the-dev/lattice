...
An interactive terminal client for the [latwallet](../../README.md) wallet
...
```
task build:latwalletcli
cd bin && ./latwalletcli
```

(When latwallet is not on `$PATH`, the create-wallet and start-daemon flows ask
for its exact location — point them at the built `latwallet`, or pass
`--latwalletbin`.)
...
- **Node & sync** — latwallet and latticed state at a glance.
- **Troubleshoot**
  - *RPC console*: run any wallet RPC (or node RPC via latwallet's passthrough)
    with method autocompletion and pretty-printed results.
  - *Doctor*: pass/warn/fail checks for config, certificates, wallet.db, TCP
    reachability, TLS, auth, sync, and latticed connectivity, with an
    exportable, credential-free report.
  - *Logs*: tail or follow the local latwallet log with level colorization.

## Connecting

latwalletcli talks to a running latwallet daemon over its legacy JSON-RPC port
...
2. Otherwise it reads the existing `~/.latwallet/latwallet.conf`
   (`username=`/`password=`, `noservertls=`, `rpclisten=`) and connects. An
   explicit `rpclisten=` is where the daemon actually listens, so latwalletcli
   dials it — whatever the active network's default port would be.
3. If there is no config at all, latwalletcli **writes a secure one itself**
   (see below) — you are never asked to make configuration choices.

The TLS certificate defaults to `~/.latwallet/rpc.cert`. Every resolution step is
...
connect target, wallet, latwallet binary), and always under `--verbose`.

## Zero-config first run

When no daemon is reachable, latwalletcli provisions and manages one for you:

- **Auto-provisioned config.** If `~/.latwallet/latwallet.conf` is missing, latwalletcli
  writes a secure one — generated RPC credentials and `usespv=1` (no latticed
  needed). TLS and listeners are left at latwallet's defaults, which are TLS on
  ...
  insecure (`noservertls`, a non-loopback listener), latwalletcli warns but
  respects it.
- **Create a wallet** (when none exists) — drives `latwallet --createfromfile`
...
- **Start latwallet now** — spawns the daemon detached (it keeps running after
  latwalletcli exits; the PID and log path are printed), waits for its RPC, and
  connects.
- **Stop the daemon** — from the **Node & sync** screen, sends latwallet's
  ...
  running latwalletcli again and choosing "Start latwallet now".

The daemon binary is resolved from `--latwalletbin` if given, otherwise from
...
the executable: latwalletcli hands wallet passphrases and seeds to the binary it
runs, so it only executes explicitly trusted paths. When latwallet is not on
...
the desktop wallet's private latwallet instance (random per-session credentials)
and steers toward running a dedicated daemon instead.

## Remote daemons

latwalletcli can drive an latwallet running on another machine:

```
latwalletcli -c wallet-host:44207 -u <user> -P <pass> --cafile <copied rpc.cert>
```

- On the daemon's machine: set a non-loopback `rpclisten=` in its
  `latwallet.conf` and open the port in the firewall. The auto-generated
  ...
  restart latwallet to regenerate.
...
- Flag-free alternative: keep a dedicated client appdata dir
  (`latwalletcli -A ~/.latwallet-remote`) whose `latwallet.conf` holds the remote
  daemon's credentials and `rpclisten=wallet-host:44207` (latwalletcli dials
...
`latwalletcli --testnet` runs the exact same flow against the test network:
credentials come from the shared `latwallet.conf`, the wallet lives in the
per-network directory (`~/.latwallet/testnet/wallet.db`), "Create a wallet" and
"Start latwallet now" pass `--testnet` through to the daemon, and the RPC port
...
`rpclisten=` pins the daemon (and latwalletcli) to that address on every
...
| Flag | Purpose |
| --- | --- |
| `-c, --connect` | Host[:port] of the latwallet RPC server (default `localhost`) |
| `-u, --rpcuser` / `-P, --rpcpass` | RPC credentials |
| `--cafile` | RPC server certificate |
| `--notls` | Disable TLS |
| `-A, --appdata` | Latwallet data directory (config/cert discovery, diagnostics) |
| `--testnet`, `--testnet2`, `--simnet`, `--signet` | Network selection |
| `--latwalletbin` | Path to the latwallet binary for wallet creation and daemon start |
| `-v, --verbose` | Trace every RPC call (method, duration, outcome) to stderr |
| `-V, --version` | Print the version |

## Secrets

latwalletcli's security posture is the same as latwallet's — both are sensitive
local services, and secrets live where latwallet keeps them (`latwallet.conf` and
the wallet DB, mode `0600`). latwalletcli adds no weaker assumptions: passphrase
...
- latwalletcli is pure Go (no cgo); it builds with `CGO_ENABLED=0` and does not
