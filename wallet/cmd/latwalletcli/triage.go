...
func runTriage(cfg *config, connectErr error) (bool, error) {
	printTitle("Cannot talk to latwallet")
	...
	if !cfg.remoteTarget() {
		...
		if confHasCredentials(cfg) && cfg.walletDBExists() && kind != triageAuth {
			opts = append(opts, huh.NewOption("Start latwallet now  spawn the daemon and connect", opStart))
		}
	}
	opts = append(opts,
	...
	case opCreate:
		...
		return true, nil
	case opStart:
		if err := startLatwalletNow(cfg); err != nil {
			printError(err)
		}
		return true, nil
	case opRetry:
...
func desktopWalletNote(cfg *config, kind triageKind) string {
	...
	return "Something is listening on " + desktopAddr + " — that looks like the desktop wallet's\n" +
		"private latwallet instance. It uses random per-session credentials, so other clients\n" +
