...
func overviewScreen(c *client) error {
	...
	for _, name := range sortedKeys(accounts) {
		rows = append(rows, [2]string{name, fmtLATT(accounts[name])})
	}
	rows = append(rows, [2]string{"total spendable", fmtLATT(spendable)})
	if pendingKnow && pending != 0 {
		rows = append(rows, [2]string{"pending", fmtLATT(pending)})
	}
