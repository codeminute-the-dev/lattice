...
// minRelayFeeRate is the network's minimum relay fee rate in LATT/kB
// (1000 cell/kB, mempool.DefaultMinRelayTxFee). latwallet's send RPCs treat the
...
func sendScreen(c *client) error {
	...
	for _, name := range names {
		label := fmt.Sprintf("%-16s %s", name, fmtLATT(accounts[name]))
		...
		huh.NewGroup(
			...
			huh.NewInput().
				...
				DescriptionFunc(func() string {
					return "Available: " + fmtLATT(accounts[fromAccount])
				}, &fromAccount).
				Placeholder("0.00 LATT").
				Validate(func(s string) error {
					amt, err := parseLATT(s)
					if err != nil {
						return err
					...
					if amt > accounts[fromAccount] {
						return fmt.Errorf("exceeds spendable balance (%s)", fmtLATT(accounts[fromAccount]))
					}
					return nil
				}).
				Value(&amountStr),
			huh.NewInput().
				...
				Description("LATT per kB; leave empty for the network minimum (0.00001).").
				Placeholder("0.00001").
	...
	amount, err := parseLATT(amountStr)
	if err != nil {
		return err
	}
...
func validateFeeRate(s string) error {
	...
	if v < minRelayFeeRate {
		return fmt.Errorf("below the network minimum relay fee (%.5f LATT/kB)", minRelayFeeRate)
	}
	return nil
}
...
func parseFeeRate(s string) (float64, string, error) {
	...
	if s == "" {
		return minRelayFeeRate, fmtLATTFloat(minRelayFeeRate) + "/kB (network minimum)", nil
	}
	v, err := strconv.ParseFloat(s, 64)
	if err != nil {
		return 0, "", fmt.Errorf("invalid fee rate")
	}
	return v, fmtLATTFloat(v) + "/kB", nil
}
