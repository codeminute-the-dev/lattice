...
// createWalletWizard creates a wallet by driving `latwallet --createfromfile`,
...
func createWalletWizard(cfg *config) error {
	...
	err = withSpinner("Creating the wallet...", func() error {
		var createErr error
		seed, createErr = runLatwalletCreate(cfg, binPath, setup)
		return createErr
	})
	if err != nil {
		return err
	...
	lipgloss.Println(th.subtle.Render("Next: pick \"Start latwallet now\" to launch the daemon and connect."))
	return nil
}

// runLatwalletCreate writes the wallet-setup JSON to a private temp file, runs
// the daemon in --createfromfile mode, and extracts the seed it prints.
func runLatwalletCreate(cfg *config, binPath string, setup map[string]string) (string, error) {
	// The setup blob carries the private passphrase and seed, and
	// latwallet --createfromfile only accepts a file path. Put it in a
	...
	dir, err := os.MkdirTemp("", "latwalletcli-setup-")
	if err != nil {
		return "", err
	}
	...
	if err != nil {
		...
		return "", fmt.Errorf("latwallet --createfromfile failed: %s", detail)
	}
...
// the birthday, and latwallet defaults a missing Bday to time.Now() — which
