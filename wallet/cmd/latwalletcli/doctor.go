...
func checkConfigFile(cfg *config) checkResult {
	path := cfg.latwalletConfPath()
	if fileExists(path) {
		return checkResult{"latwallet.conf", checkPass, path}
	}
	return checkResult{"latwallet.conf", checkWarn, path + " not found (a secure one is written automatically)"}
}
...
func checkLatwalletBinary(cfg *config) checkResult {
	if p, src, err := findLatwalletBinary(cfg); err == nil {
		return checkResult{"latwallet binary", checkPass, p + " (" + src + ")"}
	}
	return checkResult{"latwallet binary", checkWarn,
		"not on $PATH — only needed to create wallets or start the daemon from here (use --latwalletbin or the prompt)"}
}
...
func checkRPC(cfg *config, c *client) []checkResult {
	...
	results := []checkResult{{"RPC auth", checkPass, "authenticated to latwallet"}}
	...
	if info, err := c.info(); err == nil {
		results = append(results, checkResult{"latticed", checkPass,
			fmt.Sprintf("version %d, %d peer(s)", info.Version, info.Connections)})
	} else {
		results = append(results, checkResult{"latticed", checkWarn,
			"getinfo failed (latwallet cannot reach latticed): " + friendlyError(err)})
	}
	return results
}
...
func exportDoctorReport(cfg *config, results []checkResult) (string, error) {
	...
	path := fmt.Sprintf("latwalletcli-doctor-%s.txt", time.Now().Format("20060102-150405"))
