...
func TestFmtLATTFloat(t *testing.T) {
	tests := []struct {
		name string
		v    float64
		want string
	}{
		{"integral", 5, "5.00 LATT"},
		{"one decimal", 1.5, "1.50 LATT"},
		{"full precision", 0.12345678, "0.12345678 LATT"},
		{"trailing zeros trimmed", 2.50000000, "2.50 LATT"},
		{"zero", 0, "0.00 LATT"},
		{"negative", -1.5, "-1.50 LATT"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, fmtLATTFloat(tt.v))
		})
	}
}

func TestParseLATT(t *testing.T) {
	tests := []struct {
		name    string
		input   string
		want    float64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := parseLATT(tt.input)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
			require.NoError(t, err)
			assert.InDelta(t, tt.want, got.ToLATT(), 1e-8)
		})
	}
}
