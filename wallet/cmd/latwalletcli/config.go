...
import (
	...
	"github.com/codeminute-the-dev/lattice/version"
	"github.com/codeminute-the-dev/lattice/wallet/internal/cfgutil"
	"github.com/codeminute-the-dev/lattice/wallet/netparams"
	flags "github.com/jessevdk/go-flags"
)

var latwalletHomeDir = btcutil.AppDataDir("latwallet", false)

// config holds the command line options for latwalletcli.
//
// Connection settings intentionally mirror the other RPC clients in this
// repository (latctl, sweepaccount): flags always win, and anything left
// unset is discovered from the local latwallet.conf so that a default
// installation works with zero configuration.
type config struct {
	ShowVersion  bool   `short:"V" long:"version" description:"Display version information and exit"`
	Connect      string `short:"c" long:"connect" description:"Hostname[:port] of the latwallet RPC server"`
	RPCUser      string `short:"u" long:"rpcuser" description:"Latwallet RPC username"`
	RPCPass      string `short:"P" long:"rpcpass" default-mask:"-" description:"Latwallet RPC password"`
	CAFile       string `long:"cafile" description:"Certificate file used to authenticate the latwallet RPC server"`
	NoTLS        bool   `long:"notls" description:"Disable TLS for the RPC connection"`
	AppData      string `short:"A" long:"appdata" description:"Latwallet application data directory (used for config/cert discovery and diagnostics)"`
	...
	LatwalletBin string `long:"latwalletbin" description:"Path to the latwallet binary (for wallet creation and starting the daemon; default: search PATH)"`
	...
	resolvedLatwalletBin string
	resolvedLatwalletSrc string
}

// loadConfig parses command line options and fills in any unset connection
// settings from the local latwallet configuration.
func loadConfig() (*config, error) {
	cfg := &config{
		Connect:      "localhost",
		AppData:      latwalletHomeDir,
		LatwalletBin: "latwallet",
	}
	...
	if cfg.AppData != latwalletHomeDir {
		cfg.src.appData = "--appdata"
	}
	cfg.AppData = cleanAndExpandPath(cfg.AppData)

	// Fill unset credentials/TLS settings from latwallet.conf.
	cfg.src.conf = "not found"
	if fileExists(cfg.latwalletConfPath()) {
		cfg.src.conf = "found"
	}
	fromFlags := cfg.RPCUser != "" || cfg.RPCPass != ""
	fileCfg := scrapeLatwalletConf(cfg.latwalletConfPath())
	if cfg.RPCUser == "" {
	...
	if cfg.NoTLS {
		...
		cfg.NoTLS = true
		cfg.src.tls = "latwallet.conf (noservertls=1)"
	}
	if cfg.CAFile == "" {
	...
	if cfg.Connect == "localhost" {
		...
		// The daemon listens where latwallet.conf's rpclisten says, so an
		...
		for _, listen := range fileCfg.rpcListen {
			if target := dialTarget(listen, cfg.activeNet.RPCServerPort); target != "" {
				cfg.Connect = target
				cfg.src.connect = "latwallet.conf (rpclisten)"
				break
			}
		}
	}
...
// latwalletConfPath returns the daemon's configuration file location for the
// resolved appdata directory.
func (c *config) latwalletConfPath() string {
	return filepath.Join(c.AppData, "latwallet.conf")
}
...
// dialTarget converts an rpclisten value from latwallet.conf into an address a
...
// rescrapeConf re-reads latwallet.conf after latwalletcli created or amended it and
// adopts any newly available settings.
func (c *config) rescrapeConf() {
	fileCfg := scrapeLatwalletConf(c.latwalletConfPath())
	if fileCfg.username != "" {
	...
	if fileCfg.noServerTLS && !c.NoTLS {
		c.NoTLS = true
		c.src.tls = "latwallet.conf (noservertls=1)"
	}
	c.src.conf = "found"
	c.src.creds = "latwallet.conf (auto-provisioned)"
}
...
// logFilePath returns the location of the latwallet log file for the active
// network, matching latwallet's default log layout.
func (c *config) logFilePath() string {
	return filepath.Join(c.AppData, "logs", c.activeNet.Params.Name, "latwallet.log")
}

// latwalletConfValues holds settings scraped from an latwallet.conf file.
type latwalletConfValues struct {
	username    string
	password    string
	noServerTLS bool
	rpcListen   []string
...
// scrapeLatwalletConf extracts RPC credentials and server TLS/listener
// configuration from an latwallet.conf file, parsing it with the same ini
// machinery the daemon itself uses (go-flags), so quoting, comments, and
// sections behave identically. Latwallet names the auth options
// username/password, but the latticed-style rpcuser/rpcpass spelling appears in
// the wild too (latctl scrapes the latter). Missing files or fields simply
// yield zero values.
func scrapeLatwalletConf(path string) latwalletConfValues {
	var opts struct {
