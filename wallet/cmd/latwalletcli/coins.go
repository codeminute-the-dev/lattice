...
// latwalletcli started are therefore listed without an amount.
...
func coinsSummary(rows []coinRow) string {
	...
	return strings.Join([]string{
		...
		th.value.Render(fmtLATTFloat(spendable)) + th.subtle.Render(" unlocked"),
...
func coinRowLabel(row coinRow) string {
	...
	if row.known {
		amount = fmtLATTFloat(row.meta.amount)
		spend = "watchonly"
