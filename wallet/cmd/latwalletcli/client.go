...
// client wraps the JSON-RPC connection to latwallet with helpers for the latwallet
...
// dialClient establishes the HTTP POST mode RPC connection to latwallet. No
...
// (skipped when out is nil). It exists for the latwallet extension methods that
...
// getsyncprogress it works for both SPV and latticed-backed daemons.
...
// chainsynced flag plus the wallet's best block for latticed-backed daemons,
...
func (c *client) send(fromAccount, toAddress string, amount btcutil.Amount, feeRatePerKb float64, minConf int) (*chainhash.Hash, error) {
	...
	// Use sendmany rather than sendfrom: latwallet gates sendfrom on an
...
// stopDaemon asks latwallet to shut down gracefully via its authenticated
