...
// Zero-configuration provisioning: when there is no usable latwallet config,
// latwalletcli writes a secure one itself (generated credentials, loopback RPC,
...
// provisionedConf is the secure configuration latwalletcli writes when none
// exists.
...
// autoProvision ensures latwalletcli has RPC credentials to connect with or
// spawn from, choosing secure defaults with no prompts:
//
//   - If credentials are already resolved (flags or an existing latwallet.conf),
//     do nothing.
//   - Otherwise write a secure latwallet.conf: generated credentials, TLS on
//     (the latwallet default), and SPV so no latticed is needed. A missing file
//     is created; an existing file only has the missing credential keys
//     appended — other settings are never touched.
//
// No rpclisten is written: latwallet's own default is loopback-only listeners
...
func autoProvision(cfg *config) error {
	...
	confPath := cfg.latwalletConfPath()
	existing := scrapeLatwalletConf(confPath)
	pc := provisionedConf{
	...
	if created {
		printSuccess("No latwallet config found; wrote a secure one to " + confPath)
...
func writeProvisionedConf(path string, existing latwalletConfValues, pc provisionedConf) (created bool, err error) {
	...
	case errors.Is(readErr, os.ErrNotExist):
		...
		// TLS and rpclisten are deliberately left at latwallet's defaults
...
// insecureConfWarnings inspects an existing latwallet.conf for settings that
// weaken network security. latwalletcli never overrides them (they are the
// user's explicit choice) but surfaces them so the exposure is not silent.
func insecureConfWarnings(cfg *config) []string {
	v := scrapeLatwalletConf(cfg.latwalletConfPath())
	var warnings []string
	if v.noServerTLS {
		warnings = append(warnings, "latwallet.conf sets noservertls=1 — the wallet RPC runs without TLS")
	}
	for _, listen := range v.rpcListen {
		if !isLoopbackListener(listen) {
			warnings = append(warnings,
				"latwallet.conf listens on "+listen+" — the wallet RPC is reachable beyond localhost")
		}
	}
	return warnings
}
...
// confHasCredentials reports whether latwallet.conf provides a full credential
// pair, which is the precondition for spawning the daemon.
func confHasCredentials(cfg *config) bool {
	vals := scrapeLatwalletConf(cfg.latwalletConfPath())
