...
// fmtLATT renders an amount as "1.234 LATT", trimming insignificant zeros but
// always keeping at least two decimal places for scanability.
func fmtLATT(a btcutil.Amount) string {
	return fmtLATTFloat(a.ToLATT())
}

// fmtLATTFloat renders a LATT value received as a float from the RPC layer.
func fmtLATTFloat(v float64) string {
...
// parseLATT converts user input to an amount, rejecting non-positive values.
func parseLATT(s string) (btcutil.Amount, error) {
