...
func nodeStatusScreen(c *client) error {
	...
	err := withSpinner("Querying latwallet and latticed...", func() error {
		var err error
	...
	if info != nil {
		rows = append(rows,
			[2]string{"latticed version", fmt.Sprintf("%d", info.Version)},
			...
			[2]string{"Balance (1 conf)", fmtLATTFloat(info.Balance)},
			[2]string{"Relay fee", fmtLATTFloat(info.RelayFee) + "/kB"},
		)
		if info.Errors != "" {
			rows = append(rows, [2]string{"Node errors", info.Errors})
		}
	} else {
		rows = append(rows, [2]string{"latticed info", "unavailable (getinfo failed; latticed may be offline)"})
	}
...
func nodeActions(c *client) error {
	...
	ok, err := runForm(newForm(huh.NewGroup(
		huh.NewConfirm().
			Title("Stop the latwallet daemon?").
			Description("It goes offline for every client (latctl, the desktop wallet, this CLI),\nkeys are unloaded from memory, and latwalletcli will exit. Restart later by\nrunning latwalletcli again and choosing \"Start latwallet now\".").
			Affirmative("Stop it").
			Negative("Cancel").
			Value(&confirmed),
	)))
	...
	printSuccess("latwallet is stopping.")
	return errQuit
}
