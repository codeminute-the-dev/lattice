...
import (
	...
	"github.com/codeminute-the-dev/lattice/wallet/wallet/txauthor"
	"github.com/codeminute-the-dev/lattice/wallet/wallet/txrules"
	"github.com/codeminute-the-dev/lattice/wallet/wallet/txsizes"
	"github.com/jessevdk/go-flags"
	"golang.org/x/term"
)

var (
	walletDataDirectory = btcutil.AppDataDir("latwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet               bool                `long:"testnet" description:"Use the test Lattice network"`
	TestNet2              bool                `long:"testnet2" description:"Use the test Lattice network v2"`
	SimNet                bool                `long:"simnet" description:"Use the simulation Lattice network"`
	RegressionNet         bool                `long:"regtest" description:"Use the regression Lattice network"`
...
func init() {
	...
	if numNets > 1 {
		fatalf("Multiple Lattice networks may not be used simultaneously")
	}
...
func saneOutputValue(amount btcutil.Amount) bool {
	return amount >= 0 && amount <= btcutil.MaxCell
}
