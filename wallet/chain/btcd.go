...
import (
	...
	"github.com/codeminute-the-dev/lattice/node/btcutil/gcs/builder"
...
// RPCClient represents a persistent client connection to a latticed RPC server
...
type RPCClientConfig struct {
	...
	// Params defines a Lattice network by its parameters.
...
// latticed RPC server.  This channel must be continually read or the process
...
// `LatticedErrMap`, whose keys are latticed error strings and values are errors made
...
func (c *RPCClient) MapRPCErr(rpcErr error) error {
	...
	for latticedErr, matchedErr := range LatticedErrMap {
		// Match it against latticed's error.
		if matchErrStr(rpcErr, latticedErr) {
			return matchedErr
		}
	}
	...
	if !backend.SupportTestMempoolAccept() {
		...
		// the error to the older version of `latticed`.
		for latticedErr, matchedErr := range LatticedErrMapPre2402 {
			// Match it against latticed's error.
			if matchErrStr(rpcErr, latticedErr) {
				return matchedErr
			}
...
// SendRawTransaction sends a raw transaction via latticed.
