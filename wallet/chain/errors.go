...
// LatticedErrMap takes the errors returned from latticed's `testmempoolaccept` and
...
//
//nolint:lll
var LatticedErrMap = map[string]error{
	...
	// TODO(yy): find/return the following errors in `latticed`.
	//
	// A tiny transaction(in non-witness bytes) that is disallowed.
	// "unmatched latticed error 1": ErrTxTooSmall,
	// "unmatched latticed error 2": ErrScriptVerifyFlag,
	// // A transaction with invalid OP codes.
	// "unmatched latticed error 3": ErrInvalidOpcode,
	// // Minimally-small transaction(in non-witness bytes) that is
	// // allowed.
	// "unmatched latticed error 4": ErrSameNonWitnessData,

	// Returned from `testmempoolaccept` here:
	// - https://github.com/codeminute-the-dev/lattice/node/blob/d881c686e61db35e332fb0309178152dac589b03/rpcserver.go#L3893
	"missing-inputs": ErrMissingInputs,

	// Returned from `testmempoolaccept` here:
	// - https://github.com/codeminute-the-dev/lattice/node/blob/d881c686e61db35e332fb0309178152dac589b03/rpcserver.go#L3917
	"max-fee-exceeded": ErrMaxFeeExceeded,
}

// LatticedErrMapPre2402 defines the error mapping for latticed versions prior to
// 0.24.2 - all the errors changed in this commit have been defined here to
// support older versions:
// - https://github.com/codeminute-the-dev/lattice/node/pull/2053/commits/ef54c49df443815d50765e8c4f31a87944d950a6
var LatticedErrMapPre2402 = map[string]error{
	// A transaction with too large output value.
