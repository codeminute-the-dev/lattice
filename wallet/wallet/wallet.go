...
import (
	...
	"github.com/codeminute-the-dev/lattice/wallet/walletdb/migration"
...
// blockchain or latticed mempool), the next chained address is returned.
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		...
		// is calculated.
		feeF64 = (outputTotal - debitTotal).ToLATT()
	}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := btcutil.Amount(output.Value).ToLATT()
