...
Latwallet will not drop transaction history by itself, as this is something that
...
```bash
go run github.com/codeminute-the-dev/lattice/wallet/cmd/dropwtxmgr@latest
```
...
```
$ dropwtxmgr
Database path: /home/username/.latwallet/mainnet/wallet.db
Drop all Latwallet transaction history? [y/N] y
...
```
$ dropwtxmgr --db ~/.latwallet/testnet/wallet.db
Database path: /home/username/.latwallet/testnet/wallet.db
Drop all Latwallet transaction history? [y/N] y
Dropping wtxmgr namespace
```

After dropping transaction history, Latwallet may be restarted and a full rescan
will be triggered to sync the wallet:

```
$ Latwallet
14:05:31 2015-04-13 [INF] OYSTR: No recorded transaction history -- needs full rescan
...
