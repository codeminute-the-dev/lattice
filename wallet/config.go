...
import (
	...
	"github.com/codeminute-the-dev/lattice/wallet/waddrmgr"
	"github.com/codeminute-the-dev/lattice/wallet/wallet"
	flags "github.com/jessevdk/go-flags"
)

const (
	defaultCAFilename       = "latticed.cert"
	defaultConfigFilename   = "latwallet.conf"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "latwallet.log"
...
var (
	latticedDefaultCAFile = filepath.Join(btcutil.AppDataDir("latticed", false), "rpc.cert")
	defaultAppDataDir     = btcutil.AppDataDir("latwallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of latticed RPC server to connect to (default localhost:44107, testnet: localhost:44109, testnet2: localhost:44111, simnet: localhost:18556, regtest: localhost:18334)"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connection with latticed"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client"`
	LatticedUsername string                  `long:"latticedusername" description:"Username for latticed authentication"`
	LatticedPassword string                  `long:"latticedpassword" default-mask:"-" description:"Password for latticed authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy RPC and latticed authentication (if latticedusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy RPC and latticed authentication (if latticedpassword is unset)"`
...
//
// The above results in Latwallet functioning properly without any config
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.UseSPV {
		...
		if cfg.DisableClientTLS {
			...
			// If CAFile is unset, choose either the copy or local latticed cert.
			if !cfg.CAFile.ExplicitlySet() {
				cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

				// If the CA copy does not exist, check if we're connecting to
				// a local latticed and switch to its RPC cert if it exists.
				...
				if !certExists {
					if _, ok := localhostListeners[RPCHost]; ok {
						btcdCertExists, err := cfgutil.FileExists(
							latticedDefaultCAFile)
						if err != nil {
							fmt.Fprintln(os.Stderr, err)
							return nil, nil, err
						}
						if btcdCertExists {
							cfg.CAFile.Value = latticedDefaultCAFile
						}
					}
				}
			}
	...
	// If the latticed username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for latticed and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for latticed and wallet.
	if cfg.LatticedUsername == "" {
		cfg.LatticedUsername = cfg.Username
	}
	if cfg.LatticedPassword == "" {
		cfg.LatticedPassword = cfg.Password
	}
