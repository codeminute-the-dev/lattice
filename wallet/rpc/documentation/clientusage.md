...
import (
	...
	pb "github.com/codeminute-the-dev/lattice/wallet/rpc/walletrpc"
	"google.golang.org/grpc"
	"google.golang.org/grpc/credentials"
)

var certificateFile = filepath.Join(btcutil.AppDataDir("Latwallet", false), "rpc.cert")
...
```bash
$ protoc -I/path/to/lattice/wallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/lattice/wallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by Latwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.latwallet/rpc.cert"s;
    }();
    ...
    if (!status.ok()) {
        ...
        std::cout << "Spendable balance: " << response.spendable() << " Cells" << std::endl;
    }
}
```
...
```PowerShell
PS> & protoc.exe -I \Path\To\lattice\wallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by Latwallet).
            Environment.SetEnvironmentVariable("GRPC_SSL_CIPHER_SUITES", "HIGH+ECDSA");

            var walletAppData = Portability.LocalAppData(Environment.OSVersion.Platform, "Latwallet");
            ...
            try
            {
                ...
                Console.WriteLine($"Spendable balance: {balanceResponse.Spendable} Cells");
            }
            finally
            {
...
var certPath = path.join(process.env.HOME, '.latwallet', 'rpc.cert');
if (os.platform == 'win32') {
    certPath = path.join(process.env.LOCALAPPDATA, 'Latwallet', 'rpc.cert');
...
client.balance(request, function(err, response) {
    if (err) {
        ...
        console.log('Spendable balance:', response.spendable, 'Cells');
    }
});
```
...
```bash
$ protoc -I /path/to/codeminute-the-dev/lattice/wallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    cert_file_path = os.path.join(os.environ['HOME'], '.latwallet', 'rpc.cert')
    if platform.system() == 'Windows':
        cert_file_path = os.path.join(os.environ['LOCALAPPDATA'], "Latwallet", "rpc.cert")
    ...
    print 'Spendable balance: %d Cells' % response.spendable
