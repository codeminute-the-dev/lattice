...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="latwallet RPC"`)
...
// method.  This may be a request that is handled directly by Latwallet, or
// a chain server request that is handled by passing the request down to latticed.
//
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			case "authenticate":
				...
				// proxied to latticed, which would disconnect the
