...
import (
	...
	"github.com/codeminute-the-dev/lattice/node/rpcclient"
	"github.com/codeminute-the-dev/lattice/node/txscript"
	"github.com/codeminute-the-dev/lattice/node/wire"
	"github.com/codeminute-the-dev/lattice/wallet/chain"
	"github.com/codeminute-the-dev/lattice/wallet/waddrmgr"
	"github.com/codeminute-the-dev/lattice/wallet/wallet"
	"github.com/codeminute-the-dev/lattice/wallet/wallet/txrules"
	"github.com/codeminute-the-dev/lattice/wallet/wtxmgr"
)
...
var rpcHandlers = map[string]struct {
	...
	// Reference methods which can't be implemented by Latwallet due to
...
// unsupported by Latwallet due to design differences.
func unsupported(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &btcjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by Latwallet",
	}
}
...
// information about the current state of Latwallet.
// exist.
func getInfo(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	// Call down to latticed for all of the information in this command known
	// by them.
...
// appear in the blockchain, or any tx that has arrived in the latticed mempool).
...
func getUnconfirmedBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	return (bals.Total - bals.Spendable).ToLATT(), nil
}
...
func getReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	return results[acctIndex].TotalReceived.ToLATT(), nil
}
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	if len(details.Debits) == 0 {
		...
		ret.Details[0] = btcjson.GetTransactionDetailsResult{
			...
			Category: "send",
			Amount:   (-debitTotal).ToLATT(), // negative since it is a send
			Fee:      &feeF64,
		}
		ret.Fee = feeF64
	}
	...
	for _, cred := range details.Credits {
		...
		ret.Details = append(ret.Details, btcjson.GetTransactionDetailsResult{
			...
			Address:  address,
			Category: credCat,
			Amount:   cred.Amount.ToLATT(),
			Vout:     cred.Index,
		})
	}

	ret.Amount = creditTotal.ToLATT()
	return ret, nil
}
...
func help(icmd interface{}, _ *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// latticed returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to latticed.  Therefore, create a
	...
	// texts in a non-internal latticed package.
...
func listAccounts(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	for _, result := range results {
		accountBalances[result.AccountName] = result.AccountBalance.ToLATT()
	}
...
func listReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	for _, result := range results {
		jsonResults = append(jsonResults, btcjson.ListReceivedByAccountResult{
			...
			Amount:        result.TotalReceived.ToLATT(),
...
// validateAndConvertFeeRate validates the fee rate is non-negative and converts it to cells.
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// TODO: really we probably should look these up with latticed anyway to
	...
	// querying latticed with getrawtransaction. We queue up a bunch of async
