...
In the new `lattice-mining` Python package (v0.2.0), every proving function has an
...
- New package: `lattice_mining.__version__ == "0.2.0"`.
- Old package: no `lattice_mining.__version__` attribute.
...
**See `py-lattice-mining/examples/v1_v2_gateway_example.py` for a full example.**
...
Reference: `miner/lattice-gateway/src/lattice_gateway/blockchain_utils/zk_certificate.py`.
...
Reference: `py-lattice-mining/src/lib.rs` (the Python wrapper is a thin layer over
exactly these calls).
